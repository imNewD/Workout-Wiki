...
// Per 100g toggle — on by default; values entered in macro fields are
// treated as per-100g and scaled by Amount before saving.
let _intakePer100 = true;

function toggleIntakePer100() {
  _intakePer100 = !_intakePer100;
  const btn = document.getElementById('intake-per100-btn');
  if (btn) {
    btn.classList.toggle('menu-action-btn-primary', _intakePer100);
    btn.classList.toggle('menu-action-btn-ghost', !_intakePer100);
  }
  _intakeUpdateAddPreview();
}

function _intakeComputedKcal() {
  ...
  let ep = protein, ec = carbs, ef = fat;
  if (_intakePer100) {
    const mass = parseFloat(document.getElementById('intake-input-mass').value);
    if (!isNaN(mass) && mass > 0) {
      const r = mass / 100;
      ep = protein * r; ec = carbs * r; ef = fat * r;
    }
  }
  return Math.round(ep*4 + ec*4 + ef*9);
}

function _intakeUpdateKcalHint(computed) {
  const hint = document.getElementById('intake-kcal-hint');
  const kcalInput = document.getElementById('intake-input-kcal');
  if (!hint) return;
  if (!kcalInput || !kcalInput.value.trim()) { hint.style.display = 'none'; return; }
  const entered = parseFloat(kcalInput.value);
  if (isNaN(entered) || computed === 0) { hint.style.display = 'none'; return; }
  const diff = entered - computed;
  if (Math.abs(diff) < 5) { hint.style.display = 'none'; return; }
  hint.style.display = '';
  hint.textContent = diff > 0
    ? `Higher than expected (computed: ${computed} kcal from macros)`
    : `Lower than expected (computed: ${computed} kcal from macros)`;
}

function _intakeKcalInput() {
  _intakeUpdateKcalHint(_intakeComputedKcal());
}

function _intakeUpdateAddPreview() {
  const computed = _intakeComputedKcal();
  const el = document.getElementById('intakeAddKcalPreview');
  if(el) el.textContent = `= ${computed.toLocaleString()} kcal`;
  _intakeUpdateKcalHint(computed);
}
...
// Called when amount/mass changes — in per-100g mode only updates the
// preview; in absolute mode scales the input fields from the autocomplete base.
function _intakeMassInput() {
  const mass = parseFloat(document.getElementById('intake-input-mass').value);
  if (!_intakePer100 && !isNaN(mass) && mass > 0 && _intakeBaseP100 !== null) {
...
function addIntakeEntry() {
  ...
  const rawProtein = parseFloat(document.getElementById('intake-input-protein').value) || 0;
  const rawCarbs   = parseFloat(document.getElementById('intake-input-carbs').value)   || 0;
  const rawFat     = parseFloat(document.getElementById('intake-input-fat').value)     || 0;
  ...
  if(rawProtein <= 0 && rawCarbs <= 0 && rawFat <= 0) {
    _intakeShowAddError('Enter at least one macro amount (protein, carbs or fat).');
    return;
  }
  _intakeClearAddError();

  // Apply per-100g scaling to get actual macros logged
  let protein = rawProtein, carbs = rawCarbs, fat = rawFat;
  if (_intakePer100 && mass && mass > 0) {
    const ratio = mass / 100;
    protein = rawProtein * ratio;
    carbs   = rawCarbs   * ratio;
    fat     = rawFat     * ratio;
  }

  // Save to custom food memory using raw (per-100g or absolute) inputs
  if(label) {
    const inLibrary = _intakeGetFoodLibrary().some(f => f.name.toLowerCase() === label.toLowerCase());
    if(!inLibrary) _intakeAddCustomFood(label, rawProtein, rawCarbs, rawFat);
  }

  // Use custom kcal if entered, otherwise compute from macros
  const kcalInputEl = document.getElementById('intake-input-kcal');
  const customKcal = kcalInputEl ? parseFloat(kcalInputEl.value) : NaN;
  const kcal = (!isNaN(customKcal) && customKcal > 0)
  ...
  ['intake-input-label','intake-input-protein','intake-input-carbs','intake-input-fat','intake-input-mass','intake-input-kcal'].forEach(id => {
    const el = document.getElementById(id);
    if(el) el.value = '';
  });
  const hint = document.getElementById('intake-kcal-hint');
...
let _INTAKE_PENDING_REPEAT = null;
...
function repeatIntakeFromYesterday() {
  ...
  _INTAKE_PENDING_REPEAT = yesterdayEntries;
  const desc = document.getElementById('intakeRepeatConfirmDesc');
  if(desc) desc.textContent = `Add all ${yesterdayEntries.length} item${yesterdayEntries.length > 1 ? 's' : ''} from yesterday to today's log?`;
  document.getElementById('intakeRepeatConfirmOverlay')?.classList.add('open');
  document.body.style.overflow = 'hidden';
}

function _intakeCancelRepeatFromYesterday() {
  _INTAKE_PENDING_REPEAT = null;
  document.getElementById('intakeRepeatConfirmOverlay')?.classList.remove('open');
  document.body.style.overflow = '';
}

function _intakeConfirmRepeatFromYesterday() {
  const yesterdayEntries = _INTAKE_PENDING_REPEAT;
  _INTAKE_PENDING_REPEAT = null;
  document.getElementById('intakeRepeatConfirmOverlay')?.classList.remove('open');
  document.body.style.overflow = '';
  if(!yesterdayEntries || !yesterdayEntries.length) return;

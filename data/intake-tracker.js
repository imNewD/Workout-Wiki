...
function _intakeLabelInput() {
  ...
  let matches;
  if(q.length === 0) {
    matches = library.slice(0, 8);
  } else {
    const starts   = library.filter(f => f.name.toLowerCase().startsWith(q));
    const contains = library.filter(f => !f.name.toLowerCase().startsWith(q) && (f.name + ' ' + f.alt).toLowerCase().includes(q));
    matches = [...starts, ...contains].slice(0, 8);
  }
  if(matches.length === 0) { box.style.display = 'none'; return; }
  _intakeLastMatches = matches; // looked up by index on selection — avoids embedding JSON in onmousedown attrs
  box.innerHTML = matches.map((f, i) => {
    const rightTag = f.hasProfile
      ? `${f.kcal100} kcal / 100g`
    ...
    return `<div class="intake-suggest-item" onmousedown="_intakeSelectFood(${i})" style="padding:8px 12px;cursor:pointer;font-family:var(--mono);font-size:0.68rem;border-bottom:1px solid var(--border2);display:flex;justify-content:space-between;gap:8px;align-items:center">
      <span style="color:var(--text)">${_intakeEsc(f.name)}${f.alt ? ` <span style="color:var(--text3)">— ${_intakeEsc(f.alt)}</span>` : ''}</span>
      <span style="color:var(--text3);white-space:nowrap">${rightTag}</span>
    </div>`;
  }).join('');
...
function _intakeRenderMicroNote(dateKey) {
  ...
  const isToday  = dateKey === todayKey;
  const dateLabel = _intakeParseDateKey(dateKey).toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric' });
  const { loggedCount } = _intakeMicroDailyAgg(dateKey);

  let bodyHtml;
  if(loggedCount === 0) {
    bodyHtml = `<div class="empty-state" style="padding:24px 16px"><span class="empty-icon">💊</span>No profiled food logged ${isToday ? 'today' : 'on this day'} — nothing to grade yet.</div>`;
  } else {
    const missed = _intakeMicroMissedList(dateKey);
    if(missed.length === 0) {
      bodyHtml = `<div class="empty-state" style="padding:24px 16px"><span class="empty-icon">✅</span>Nothing missed — every tracked micronutrient is at 75%+ of its daily value.</div>`;
    } else {
      const rows = missed.map(r => {
        const color = r.daily < 50 ? '#e63946' : '#ffae1f';
        return `<div class="intake-micro-note-row">
          <span class="intake-micro-note-label">${_intakeEsc(r.label)}</span>
          <span class="intake-micro-note-pct" style="color:${color}">${r.daily}%</span>
        </div>`;
      }).join('');
      bodyHtml = `
        <div class="intake-micro-note">${missed.length} nutrient${missed.length > 1 ? 's' : ''} under 75% of daily value:</div>
        <div class="intake-micro-note-list">${rows}</div>
      `;
    }
  }
  ...
  card.innerHTML = `
    <div class="intake-target-hdr">
      <h3>${isToday ? "Today's Gaps" : _intakeEsc(dateLabel)}</h3>
      ${isToday ? '' : `<button class="menu-action-btn menu-action-btn-ghost menu-action-btn-sm" onclick="closeIntakeMicroNote()">✕ TODAY</button>`}
    </div>
    ${bodyHtml}
  `;
  card.style.display = '';
...
function _intakeRender() {
  INTAKE_CAL_STATE.selectedDate = null;
  document.querySelectorAll('#intakeModeSwitch .tree-mode-btn').forEach(b => b.classList.toggle('active', b.dataset.intakeMode === INTAKE_STATE.mode));
  document.getElementById('intakeMacroPanel').style.display = INTAKE_STATE.mode === 'macro' ? '' : 'none';
  document.getElementById('intakeMicroPanel').style.display = INTAKE_STATE.mode === 'micro' ? '' : 'none';
  _intakeRenderTargetCard();
  _intakeRenderEntries();
  _intakeRenderCalendar();
  if(INTAKE_STATE.mode === 'micro') _intakeRenderMicro();
  const feedbackCard = document.getElementById('intakeDayFeedbackCard');
  if(feedbackCard) feedbackCard.style.display = 'none';
}

document.addEventListener('DOMContentLoaded', () => {
  if(document.getElementById('view-intake-tracker')) _intakeRender();
});

/* ══ END INTAKE TRACKER MODULE ══════════════════════════════════════
   Extracted from index.html. Smoke-test after touching this file:
   switch Macro/Micro tabs, add + delete an entry, toggle the target
   override, flip calendar months on both panels, click a past
   calendar day, "Repeat from Yesterday", refresh and confirm
   everything persisted (same localStorage keys as before the move). */

...
<text x="${(W-pr-2).toFixed(1)}" y="${(+yP(target)-6).toFixed(1)}" fill="var(--green)" font-size="8" text-anchor="end" font-family="var(--mono)">GOAL ${target}${unit}</text>`:'';
  ...
  function renderMetrics() {
    ...
    // Reset APD suggestions so they regenerate fresh
    ...
    el.querySelector('#metrics-content').innerHTML = `
      ...
      <div class="db-section">
        <div class="db-section-hd">
          <span class="db-section-title">PR BOARD</span>
  ...
  function _apdBuildSuggestion(type) {
    ...
    /* ── WEAK POINTS ── */
      ...
      const totals = Object.fromEntries(Object.entries(groups).map(([g,ms])=>[g, ms.reduce((s,m)=>s+(md[m]||0),0)]));
      const sorted = Object.entries(totals).sort((a,b)=>a[1]-b[1]);
      const weakest = sorted[0][0], weak2 = sorted[1]?.[0] || 'Core';
      const grand = Object.values(totals).reduce((a,b)=>a+b,0);
      const pctWeak = grand ? Math.round(totals[weakest]/grand*100) : 0;
  ...
  function _apdRenderSuggestions() {
    if (!_apdSuggestions) {
      ...
      _apdSuggestions = types.map(t => _apdBuildSuggestion(t));
    }
  ...
  window.apdGoalSearch = function (query) {
    ...
    const q = query.trim();
    // Search across all LIB_DATA keys except pantheon
    const excluded = new Set(['pantheon']);
    const allExercises = [];
    if (typeof LIB_DATA !== 'undefined') {
      Object.keys(LIB_DATA).forEach(function(k) {
        if (excluded.has(k)) return;
        (LIB_DATA[k] || []).forEach(function(ex) {
          allExercises.push({ name: ex.name || ex.alt || '', libKey: k, diff: ex.diff });
        });
      });
    }
  ...
  window.apdSaveFromPreview = function() {
    ...
    _apdDoSave(name, _apdEditState.sessions, s.id, s.days, s.why);
    window.apdClosePreview();
  };

  // Alias so any stale onclick="apdGenerate(...)" still works
  window.apdGenerate = window.apdPreview;

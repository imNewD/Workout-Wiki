...
<text x="${(W-pr-2).toFixed(1)}" y="${(+yP(target)-6).toFixed(1)}" fill="var(--green)" font-size="8" text-anchor="end" font-family="var(--mono)">GOAL ${target}${unit}</text>`:'';
  ...
  window.metricsSubmitLogPR = function () {
    ...
    // All-time best BEFORE this entry — a genuine "record broken" is beating
    // every past value, not just the previous one (that's what the milestone
    // badges below check, which is a different, looser thing).
    const priorBest = pr.entries.length ? Math.max(...pr.entries.map(e => e.v)) : null;
    const isNewRecord = priorBest !== null && v > priorBest;
    // Milestone badges
    let badge = null;
    ...
    metricsCloseLogPR();
    renderMetrics();
    if (isNewRecord && typeof window.recordPRBreak === 'function') window.recordPRBreak();
    if (badge) {
...
(function () {
  ...
  function requestPermission() {
    ...
    return Notification.requestPermission().then(p => {
      _renderPerm();
      if (p === 'granted') _registerBackgroundWake();
      return p;
    });
  }
  ...
  /* ─── Background wake registration ────────────────────────────
     Periodic Background Sync (installed PWA on Chromium/Android)
     lets the browser wake sw.js to fire alarms with no tab open.
     Everywhere else it's a silent no-op and the SW's own re-arm-on-
     wake logic still covers backgrounded tabs + next-launch firing. */
  function _registerBackgroundWake() {
    const p = _swReady();
    if (!p) return;
    p.then(reg => {
      if ('periodicSync' in reg) {
        reg.periodicSync.register('grnd-alarm-check', { minInterval: 15 * 60 * 1000 }).catch(() => {});
      }
    }).catch(() => {});
  }
  ...
  function _init() {
    ...
    _registerBackgroundWake();

    // Re-sync every time the app goes to the background — the SW copy
    // is what fires when this page gets frozen or closed.
    document.addEventListener('visibilitychange', () => {
      if (document.visibilityState === 'hidden') {
        const left = _load().filter(a => a.fireAt > Date.now());
        if (left.length) _swSend({ type: 'SYNC_ALARMS', alarms: left });
      }
    });

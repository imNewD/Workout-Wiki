...
(function(){
  ...
  var LIBS = [
    ...
    { v:'chinup-library',     libKey:'chinup',     section:'bodyweight', searchable:true,
      parent:['bodyweight','BODYWEIGHT'], leaf:'CHIN-UP LIBRARY', tagSub:DIFF_SUB,
...
function updateHomeBodyweightSummary() {
  ...
  const bwEl = document.getElementById('home-bodyweight-count');
  if(bwEl) bwEl.textContent = bwCount + ' exercises';
  ...
  const cardioEl = document.getElementById('home-cardio-count');
  if(cardioEl) cardioEl.textContent = cardioCount + ' exercises';

  // Weights count
  const weightsCount = ['gymChest','gymBack','gymShoulders','gymLegs','gymArms','gymCore','weighted'].reduce((s,k) => s + (LIB_DATA[k]||[]).length, 0);
  const weightsEl = document.getElementById('home-weights-count');
  if(weightsEl) weightsEl.textContent = weightsCount + ' exercises';

...
function initLib(libKey) {
  ...
  // Search — debounced: renderLibTable rebuilds the whole table (can take
  // 100ms-plus on the combined "all" libraries), so running it on every
  // keystroke freezes typing. Wait for a short pause instead.
...
/* ═══════════════════════════════════════════════════════════════
   SEASONAL THEME AMBIENT FX  🌊❄🌱🍂
   Four particle systems modeled on the sidequests/beber pattern:
   spawn-few-WAAPI-elements → auto-remove → reschedule. Each one
   only runs while its theme is active (MutationObserver sync),
   waits out the loading screen, respects prefers-reduced-motion,
   and keeps element counts tiny so weak devices stay smooth.
═══════════════════════════════════════════════════════════════ */

/* Shared helper — one media query check for all four systems. */
function _grndReducedMotion(){
  try { return window.matchMedia('(prefers-reduced-motion: reduce)').matches; }
  catch(e){ return false; }
}

/* Particle-density helper — seasonal FX honour the High-Performance
   toggle (data-ui-power="high"). Lite mode spawns roughly half the
   particles at longer intervals so weak devices stay smooth. */
function _grndFxN(base, spread){
  var high = document.documentElement.dataset.uiPower === 'high';
  var n = base + Math.floor(Math.random() * (spread + 1));
  return high ? n : Math.max(1, Math.round(n * 0.5));
}
function _grndFxIv(ms){
  return document.documentElement.dataset.uiPower === 'high' ? ms : ms * 1.7;
}

/* ══════════════════════════════════════════════════════════════
   SEASONAL SUN ENGINE  ☀  — the real sky, on your clock
   ──────────────────────────────────────────────────────────────
   While a seasonal theme is active, this engine reads the DEVICE
   CLOCK once a minute and drives the scene's sun:
     • --sun-x / --sun-y   — the sun disc's position (rises in the
                             east/left, arcs overhead, sets right).
                             Winter's arc stays LOW — a cold sun
                             that never really climbs.
     • --sun-core / --sun-halo-rgb / --sun-op — disc & glow colours
                             per phase (warm at dawn, white at noon,
                             deep orange at dusk, gone at night).
     • data-sun-phase      — "dawn" | "day" | "dusk" | "night" on
                             <html>; themes.css re-lights the whole
                             scene from it (tint overlay + stars).
   Each season has its own realistic day length (summer ~16h,
   winter ~8h). Debug/preview: GRNDSun.force(hour) pins the clock
   (e.g. GRNDSun.force(21.8) → dusk), GRNDSun.force(null) resumes.
══════════════════════════════════════════════════════════════ */
(function(){
  var SEASONAL = { summer:1, winter:1, spring:1, autumn:1 };

  /* rise/set = fractional local hours · apexY = %-from-top at solar
     noon (higher number = lower sun) · horizonY = where the disc
     meets the ground/sea line in each theme's scene. */
  var SUN = {
    summer: { rise: 5.5,  set: 21.5, apexY: 9,  horizonY: 46 },
    winter: { rise: 8.1,  set: 16.4, apexY: 58, horizonY: 78 },
    spring: { rise: 6.4,  set: 19.6, apexY: 14, horizonY: 53 },
    autumn: { rise: 7.2,  set: 18.4, apexY: 22, horizonY: 50 },
  };
  /* Disc + halo colours per season & phase: [core, haloRGB] */
  var PAL = {
    summer: { dawn:['#ffd9a8','255,150,90'],  day:['#fffbe8','255,236,170'], dusk:['#ffae5e','255,110,60']  },
    winter: { dawn:['#f2e0d0','220,190,170'], day:['#f4f8fc','200,220,240'], dusk:['#f0c8a8','210,150,120'] },
    spring: { dawn:['#ffe4b8','255,180,110'], day:['#fffdf0','255,244,190'], dusk:['#ffc078','255,140,80']  },
    autumn: { dawn:['#f8d8a8','230,170,110'], day:['#faf4e0','240,225,180'], dusk:['#f0a860','220,120,60']  },
  };

  var _timer = null;
  var _forced = null;   // forced fractional hour for preview/debug
  ...
  function _clear(){
    ...
    ['--sun-x','--sun-y','--sun-core','--sun-halo-rgb','--sun-op'].forEach(function(v){ st.removeProperty(v); });
    delete document.documentElement.dataset.sunPhase;
  }

  function refresh(){
    var theme = document.documentElement.dataset.theme;
    if (!SEASONAL[theme]) { _clear(); return; }

    var cfg = SUN[theme], pal = PAL[theme];
    var t = _now();
    var phase;
    if (t < cfg.rise - 0.9 || t > cfg.set + 0.9)      phase = 'night';
    else if (t < cfg.rise + 1.1)                       phase = 'dawn';
    else if (t > cfg.set - 1.1)                        phase = 'dusk';
    else                                               phase = 'day';

    /* Arc: f 0→1 across the day; x sweeps left→right, y follows a
       sine arc from the horizon up to the season's apex and back. */
    var f = (t - cfg.rise) / (cfg.set - cfg.rise);
    var fc = Math.max(0.02, Math.min(0.98, f));
    var x = 12 + fc * 76;
    var y = cfg.horizonY - Math.sin(Math.PI * fc) * (cfg.horizonY - cfg.apexY);

    /* Fade the disc in/out through dawn/dusk; gone at night. */
    var op = 1;
    if (phase === 'night') op = 0;
    else if (phase === 'dawn') op = Math.max(0.15, Math.min(1, (t - (cfg.rise - 0.9)) / 1.2));
    else if (phase === 'dusk') op = Math.max(0.15, Math.min(1, ((cfg.set + 0.9) - t) / 1.2));

    var colors = pal[phase === 'night' ? 'dusk' : phase];
    var st = document.documentElement.style;
    st.setProperty('--sun-x', x.toFixed(1) + '%');
    st.setProperty('--sun-y', y.toFixed(1) + '%');
    st.setProperty('--sun-core', colors[0]);
    st.setProperty('--sun-halo-rgb', colors[1]);
    st.setProperty('--sun-op', op.toFixed(2));
    document.documentElement.dataset.sunPhase = phase;
  }

  function _schedule(){
    clearInterval(_timer);
    _timer = setInterval(refresh, 60000);
  }

  /* The day-phase tint overlay — one fixed div themes.css restyles
     per phase (dawn glow / dusk fire / night dark + stars). */
  function _ensureDaylightLayer(){
    if (!document.body || document.getElementById('season-daylight')) return;
    var d = document.createElement('div');
    d.id = 'season-daylight';
    d.setAttribute('aria-hidden','true');
    document.body.appendChild(d);
  }

  function sync(){
    _ensureDaylightLayer();
    refresh();
    if (SEASONAL[document.documentElement.dataset.theme]) _schedule();
  ...
  window.GRNDSun = {
    refresh: refresh,
    force: function(hour){ _forced = (hour == null ? null : +hour); refresh(); }
  };
})();

/* ══ SUMMER — sea sparkles, a sailboat on the horizon, gulls ══ */
(function(){
  var THEME = 'summer';
  var overlay = null, running = false;
  var sparkTimer=null, boatTimer=null, gullTimer=null, startTimer=null;
  var waveTimer=null, cloudTimer=null;

  function rand(a,b){ return a + Math.random()*(b-a); }
  function ensureOverlay(){
    if(overlay) return overlay;
    overlay = document.getElementById('summer-fx-overlay');
    if(!overlay){
      overlay = document.createElement('div');
      overlay.id = 'summer-fx-overlay';
      overlay.setAttribute('aria-hidden','true');
      overlay.style.cssText = 'position:fixed; inset:0; overflow:hidden; pointer-events:none; z-index:9999;';
      document.body.appendChild(overlay);
    }
  ...
  /* Scene layer BEHIND the content (z1 < content z2): clouds, the
     sailboat, gulls and surf live in the world, not over the UI. */
  var sky = null;
  function ensureSky(){
    if(sky) return sky;
    sky = document.getElementById('summer-sky-overlay');
    if(!sky){
      sky = document.createElement('div');
      sky.id = 'summer-sky-overlay';
      sky.setAttribute('aria-hidden','true');
      sky.style.cssText = 'position:fixed; inset:0; overflow:hidden; pointer-events:none; z-index:1;';
      document.body.appendChild(sky);
    }
    return sky;
  }
  function clearOverlay(){ if(overlay) overlay.innerHTML=''; if(sky) sky.innerHTML=''; }
  function gone(el,ms){ setTimeout(function(){ if(el&&el.parentNode) el.parentNode.removeChild(el); }, ms); }

  /* Sun glitter dancing on the water band (47%–67% of the viewport). */
  function spawnSparkles(){
    if(!running || document.documentElement.dataset.theme !== THEME) return;
    ensureOverlay();
    var count = _grndFxN(4, 3);
    for(var i=0;i<count;i++){
      var s = document.createElement('span');
      s.className = 'summer-fx summer-sparkle';
      var size = rand(2,5);
      var x = rand(window.innerWidth*0.05, window.innerWidth*0.95);
      var y = rand(window.innerHeight*0.47, window.innerHeight*0.67);
      var dur = rand(1600, 3400);
      s.style.cssText = 'position:absolute; left:0; top:0; width:'+size+'px; height:'+size+'px; border-radius:50%; '+
        'background:radial-gradient(circle, rgba(255,255,240,0.98) 0%, rgba(255,244,190,0.7) 50%, rgba(255,240,180,0) 80%); '+
        'box-shadow:0 0 '+(size*2.4)+'px rgba(255,248,210,0.85); '+
        'transform:translate('+x+'px,'+y+'px) scale(0.3);';
      overlay.appendChild(s);
      s.animate([
        { opacity:0, transform:'translate('+x+'px,'+y+'px) scale(0.3)' },
        { opacity:1, transform:'translate('+(x+rand(-6,6))+'px,'+y+'px) scale(1)', offset:0.45 },
        { opacity:0, transform:'translate('+(x+rand(-10,10))+'px,'+y+'px) scale(0.35)', offset:1 }
      ], { duration:dur, easing:'ease-in-out', fill:'forwards' });
      gone(s, dur+150);
    }
    scheduleSparkles();
  ...
  function scheduleSparkles(){
    if(!running || document.documentElement.dataset.theme !== THEME) return;
    clearTimeout(sparkTimer);
    sparkTimer = setTimeout(spawnSparkles, _grndFxIv(rand(1200, 2400)));
  }

  /* A little sailboat crossing the horizon line, once in a while —
     floats on the swell rather than flying straight across: gentle
     vertical buoyancy, a slight rocking roll, eased accel/decel. */
  function spawnBoat(){
    if(!running || document.documentElement.dataset.theme !== THEME){ scheduleBoat(); return; }
    ensureOverlay();
    var dir = Math.random() < 0.5 ? 1 : -1;
    var w = rand(26, 40), h = w*0.9;
    var y = window.innerHeight*0.462 - h*0.35;
    var startX = dir>0 ? -w-20 : window.innerWidth+20;
    var endX   = dir>0 ? window.innerWidth+20 : -w-20;
    var dur = rand(38000, 55000);
    var boat = document.createElement('span');
    boat.className = 'summer-fx';
    boat.style.cssText = 'position:absolute; left:0; top:0; width:'+w+'px; height:'+h+'px; opacity:0; '+
      'transform:translate('+startX+'px,'+y+'px)'+(dir<0?' scaleX(-1)':'')+';';
    boat.innerHTML = '<svg viewBox="0 0 40 36" width="'+w+'" height="'+h+'">'+
      '<path d="M20 2 L20 24 L6 24 Z" fill="rgba(255,255,255,0.92)"/>'+
      '<path d="M22 6 L22 24 L33 24 Z" fill="rgba(240,250,255,0.85)"/>'+
      '<path d="M4 26 L36 26 L30 33 L10 33 Z" fill="rgba(30,60,90,0.85)"/>'+
      '</svg>';
    ensureSky().appendChild(boat);
    var flip = dir<0 ? ' scaleX(-1)' : '';
    var bobAmp = rand(2.5, 4);      // vertical buoyancy, px
    var rollAmp = rand(2, 3.5);     // rocking roll, deg
    var cycles = Math.max(3, Math.round(dur/8500)); // slow, wave-paced
    var N = 28;
    var frames = [];
    for(var i=0;i<N;i++){
      var t = i/(N-1);
      var ease = 0.5 - Math.cos(Math.PI*t)/2; // ease-in-out-sine: slow start/end
      var bx = startX + (endX-startX)*ease;
      var by = y + Math.sin(t*Math.PI*2*cycles)*bobAmp;
      var roll = Math.sin(t*Math.PI*2*cycles*0.8 + 0.6)*rollAmp;
      var op = t<0.05 ? (t/0.05)*0.9 : (t>0.95 ? ((1-t)/0.05)*0.9 : 0.9);
      frames.push({ transform:'translate('+bx.toFixed(2)+'px,'+by.toFixed(2)+'px)'+flip+' rotate('+roll.toFixed(2)+'deg)', opacity:op, offset:t });
    }
    boat.animate(frames, { duration:dur, easing:'linear', fill:'forwards' });
    gone(boat, dur+200);
    scheduleBoat();
  }
  function scheduleBoat(){
    ...
    boatTimer = setTimeout(spawnBoat, rand(30000, 55000));
  }

  /* Gulls gliding across the sky in a loose pair. */
  function spawnGulls(){
    if(!running || document.documentElement.dataset.theme !== THEME){ scheduleGulls(); return; }
    ensureOverlay();
    var dir = Math.random() < 0.5 ? 1 : -1;
    var n = 1 + Math.floor(Math.random()*2);
    for(var i=0;i<n;i++){
      var g = document.createElement('span');
      g.className = 'summer-fx';
      var s = rand(14, 22);
      var y0 = rand(window.innerHeight*0.08, window.innerHeight*0.30);
      var startX = dir>0 ? -40 : window.innerWidth+40;
      var travel = (window.innerWidth+80) * dir;
      var dur = rand(14000, 20000);
      var bob = rand(14, 30);
      g.style.cssText = 'position:absolute; left:0; top:0; opacity:0; transform:translate('+startX+'px,'+y0+'px);';
      g.innerHTML = '<svg viewBox="0 0 24 10" width="'+s+'" height="'+(s*0.42)+'">'+
        '<path d="M1 8 Q6 1 12 6 Q18 1 23 8" fill="none" stroke="rgba(40,60,80,0.75)" stroke-width="1.6" stroke-linecap="round"/></svg>';
      ensureSky().appendChild(g);
      g.animate([
        { transform:'translate('+startX+'px,'+y0+'px)', opacity:0 },
        { opacity:0.85, offset:0.08 },
        { transform:'translate('+(startX+travel*0.35)+'px,'+(y0-bob)+'px)', offset:0.35 },
        { transform:'translate('+(startX+travel*0.7)+'px,'+(y0+bob*0.5)+'px)', opacity:0.85, offset:0.7 },
        { transform:'translate('+(startX+travel)+'px,'+(y0-bob*0.4)+'px)', opacity:0 }
      ], { duration:dur, delay:i*rand(600,1400), easing:'ease-in-out', fill:'forwards' });
      gone(g, dur+2000);
    }
    scheduleGulls();
  }
  function scheduleGulls(){
    ...
    gullTimer = setTimeout(spawnGulls, rand(16000, 30000));
  }

  /* A wave washing up the beach: a full-width foam band surges up
     over the sand and pulls back, leaving a fading sheen. */
  function spawnWave(){
    if(!running || document.documentElement.dataset.theme !== THEME){ scheduleWave(); return; }
    ensureOverlay();
    var y = window.innerHeight * 0.695;
    var surge = window.innerHeight * rand(0.018, 0.034);
    var dur = rand(4200, 6200);
    var wave = document.createElement('span');
    wave.className = 'summer-fx';
    // Scalloped foam crest (a repeating tile of soft bumps) sitting on the
    // wash gradient — a flat blurred bar alone read as a smudged white
    // patch with no wave texture at all.
    wave.style.cssText = 'position:absolute; left:-2%; top:'+y+'px; width:104%; height:22px; '+
      'background-image:'+
        'radial-gradient(circle at 50% 100%, rgba(255,255,255,0.95) 0 44%, rgba(255,255,255,0.3) 50%, transparent 58%),'+
        'linear-gradient(180deg, rgba(255,255,255,0) 0%, rgba(255,255,255,0.6) 52%, rgba(225,247,252,0.32) 78%, rgba(255,255,255,0) 100%);'+
      'background-size: 26px 22px, 100% 100%; '+
      'background-repeat: repeat-x, no-repeat; '+
      'background-position: 0 0, 0 0; '+
      'filter:blur(0.5px); opacity:0;';
    ensureSky().appendChild(wave);
    wave.animate([
      { transform:'translateY(0) scaleY(0.6)',              opacity:0 },
      { transform:'translateY('+(surge*0.55)+'px) scaleY(1)',   opacity:0.9, offset:0.3 },
      { transform:'translateY('+surge+'px) scaleY(1.25)',       opacity:0.75, offset:0.55 },
      { transform:'translateY('+(surge*0.3)+'px) scaleY(0.8)',  opacity:0.35, offset:0.8 },
      { transform:'translateY(0) scaleY(0.55)',             opacity:0 }
    ], { duration:dur, easing:'ease-in-out', fill:'forwards' });
    setTimeout(function(el){ if(el && el.parentNode) el.parentNode.removeChild(el); }, dur+200, wave);
    scheduleWave();
  }
  function scheduleWave(){
    if(!running || document.documentElement.dataset.theme !== THEME) return;
    clearTimeout(waveTimer);
    waveTimer = setTimeout(spawnWave, _grndFxIv(rand(5200, 9500)));
  }

  /* Fat white cumulus drifting across the summer sky. */
  function spawnCloud(){
    if(!running || document.documentElement.dataset.theme !== THEME){ scheduleCloud(); return; }
    ensureOverlay();
    var dir = Math.random()<0.5 ? 1 : -1;
    var w = rand(140, 300);
    var h = w * rand(0.3, 0.4);
    var y = rand(window.innerHeight*0.04, window.innerHeight*0.28);
    var startX = dir>0 ? -w : window.innerWidth;
    var endX = dir>0 ? window.innerWidth : -w;
    var dur = rand(40000, 65000);
    var cloud = document.createElement('span');
    cloud.className = 'summer-fx';
    cloud.style.cssText = 'position:absolute; left:0; top:0; width:'+w+'px; height:'+h+'px; '+
      'background:'+
        'radial-gradient(38% 62% at 30% 62%, rgba(255,255,255,0.94) 0 58%, rgba(255,255,255,0) 78%),'+
        'radial-gradient(46% 78% at 52% 44%, rgba(255,255,255,0.98) 0 58%, rgba(255,255,255,0) 78%),'+
        'radial-gradient(34% 58% at 74% 60%, rgba(255,255,255,0.9) 0 58%, rgba(255,255,255,0) 78%); '+
      'filter:blur(2px); transform:translate('+startX+'px,'+y+'px);';
    ensureSky().appendChild(cloud);
    cloud.animate([
      { transform:'translate('+startX+'px,'+y+'px)', opacity:0 },
      { opacity:0.95, offset:0.07 },
      { opacity:0.95, offset:0.92 },
      { transform:'translate('+endX+'px,'+(y+rand(-8,8))+'px)', opacity:0 }
    ], { duration:dur, easing:'linear', fill:'forwards' });
    setTimeout(function(el){ if(el && el.parentNode) el.parentNode.removeChild(el); }, dur+200, cloud);
    scheduleCloud();
  }
  function scheduleCloud(){
    if(!running || document.documentElement.dataset.theme !== THEME) return;
    clearTimeout(cloudTimer);
    cloudTimer = setTimeout(spawnCloud, _grndFxIv(rand(12000, 24000)));
  }

  function start(){
    if(running) return;
    if(document.documentElement.dataset.theme !== THEME) return;
    if(_grndReducedMotion()) return;
    if(document.getElementById('grnd-loader')){
      clearTimeout(startTimer);
      startTimer = setTimeout(start, 350);
      return;
    ...
    running = true;
    ensureOverlay(); clearOverlay();
    spawnSparkles();
    boatTimer  = setTimeout(spawnBoat, rand(4000, 9000));
    gullTimer  = setTimeout(spawnGulls, rand(6000, 12000));
    waveTimer  = setTimeout(spawnWave, rand(1500, 3500));
    cloudTimer = setTimeout(spawnCloud, rand(2000, 6000));
  }
  function stop(){
    running = false;
    clearTimeout(sparkTimer); clearTimeout(boatTimer);
    clearTimeout(gullTimer); clearTimeout(startTimer);
    clearTimeout(waveTimer); clearTimeout(cloudTimer);
    clearOverlay();
  }
  function sync(){ if(document.documentElement.dataset.theme === THEME) start(); else stop(); }
  if(document.readyState === 'loading') document.addEventListener('DOMContentLoaded', sync);
  else sync();
  new MutationObserver(sync).observe(document.documentElement, { attributes:true, attributeFilter:['data-theme'] });
})();
...
/* ══ WINTER — falling snow, click flurries, drifting fog ══════
   Continuous gentle snowfall; interacting with the app (opening
   or closing tabs, cards, menus) shakes loose a little snow-puff
   burst from the element you touched. */
(function(){
  var THEME = 'winter';
  var overlay = null, running = false;
  var snowTimer=null, fogTimer=null, startTimer=null, pileTimer=null;
  /* Cards that collect snow while left untouched. */
  var PILE_SELECTOR = '.prog-card,.cat-card,.detail-block,.detail-row,.detail-cell,.lib-item,.nutrition-card,.prog-group,.wip-card';
  var PILE_CAP_PER_CARD = 18;
  /* Elements that count as "tabs" — nav, cards, list items, closers. */
  var CLICK_SELECTOR = '.bnav-back,.bnav-home,.bnav-menu,.prog-card,.cat-card,.lib-item,'+
    '.modal-close,.jb-close,.header-menu-btn,.tree-mode-btn,.theme-pill-opt,.menu-setting-row,'+
    '.tier-header,.filter-chip,.view-toggle-btn,.menu-action-btn';

  function rand(a,b){ return a + Math.random()*(b-a); }
  function ensureOverlay(){
    if(overlay) return overlay;
    overlay = document.getElementById('winter-fx-overlay');
    if(!overlay){
      overlay = document.createElement('div');
      overlay.id = 'winter-fx-overlay';
      overlay.setAttribute('aria-hidden','true');
      overlay.style.cssText = 'position:fixed; inset:0; overflow:hidden; pointer-events:none; z-index:9999;';
      document.body.appendChild(overlay);
    }
  ...
  function makeFlake(size, x, y, glow){
    var f = document.createElement('span');
    f.className = 'winter-flake';
    f.style.cssText = 'position:absolute; left:0; top:0; width:'+size+'px; height:'+size+'px; border-radius:50%; '+
      'background:radial-gradient(circle at 40% 36%, rgba(255,255,255,0.98) 0%, rgba(228,242,252,0.85) 55%, rgba(220,238,252,0) 82%); '+
      (glow ? 'box-shadow:0 0 '+(size*1.6)+'px rgba(210,235,255,0.55); ' : '')+
      'transform:translate('+x+'px,'+y+'px);';
    return f;
  }

  /* Continuous snowfall — small batches, long falls, gentle sway. */
  function spawnSnow(){
    if(!running || document.documentElement.dataset.theme !== THEME) return;
    ensureOverlay();
    var count = _grndFxN(5, 4);
    for(var i=0;i<count;i++){
      var size = rand(2.5, 6.5);
      var x = rand(-20, window.innerWidth+20);
      var y = -rand(10, 60);
      var fallTo = window.innerHeight + 20;
      var sway = rand(24, 70) * (Math.random()<0.5?-1:1);
      var dur = rand(9000, 17000);
      var op = rand(0.5, 0.95);
      var f = makeFlake(size, x, y, size>5);
      overlay.appendChild(f);
      f.animate([
        { transform:'translate('+x+'px,'+y+'px)', opacity:0 },
        { opacity:op, offset:0.06 },
        { transform:'translate('+(x+sway)+'px,'+(y+(fallTo-y)*0.4)+'px)', offset:0.4 },
        { transform:'translate('+(x-sway*0.6)+'px,'+(y+(fallTo-y)*0.75)+'px)', opacity:op, offset:0.75 },
        { transform:'translate('+(x+sway*0.4)+'px,'+fallTo+'px)', opacity:0 }
      ], { duration:dur, easing:'linear', fill:'forwards' });
      gone(f, dur+200);
    }
    scheduleSnow();
  ...
  function scheduleSnow(){
    ...
    snowTimer = setTimeout(spawnSnow, _grndFxIv(rand(1400, 2600)));
  }

  /* Snow-puff burst when a tab/card is opened or closed — flakes
     shake loose from the top edge of the element and drift down. */
  function burstAt(rect){
    if(!running || document.documentElement.dataset.theme !== THEME) return;
    ensureOverlay();
    var count = _grndFxN(7, 5);
    for(var i=0;i<count;i++){
      var size = rand(2, 5);
      var x = rand(rect.left, rect.right);
      var y = rect.top + rand(-4, 8);
      var driftX = rand(-34, 34);
      var fall = rand(40, 110);
      var dur = rand(900, 1900);
      var f = makeFlake(size, x, y, false);
      overlay.appendChild(f);
      f.animate([
        { transform:'translate('+x+'px,'+y+'px) scale(0.4)', opacity:0 },
        { transform:'translate('+(x+driftX*0.4)+'px,'+(y+fall*0.25)+'px) scale(1)', opacity:0.9, offset:0.25 },
        { transform:'translate('+(x+driftX)+'px,'+(y+fall)+'px) scale(0.7)', opacity:0 }
      ], { duration:dur, easing:'ease-out', fill:'forwards' });
      gone(f, dur+150);
    }
  }
  /* Shake the settled snow off a card: every snowball it collected
     tumbles off and falls away. */
  function shakeOff(el){
    var piles = el.querySelectorAll('.winter-snowpile');
    piles.forEach(function(b){
      var dx = rand(-26, 26);
      var fall = rand(30, 80);
      var dur = rand(600, 1100);
      var anim = b.animate([
        { transform:'translate(0,0) scale(1)', opacity:1 },
        { transform:'translate('+dx+'px,'+fall+'px) scale(0.6)', opacity:0 }
      ], { duration:dur, easing:'ease-in', fill:'forwards' });
      anim.onfinish = function(){ if(b.parentNode) b.parentNode.removeChild(b); };
      setTimeout(function(){ if(b.parentNode) b.parentNode.removeChild(b); }, dur+200);
    });
  }

  document.addEventListener('click', function(e){
    if(!running || document.documentElement.dataset.theme !== THEME) return;
    var el = e.target && e.target.closest ? e.target.closest(CLICK_SELECTOR) : null;
    var pileEl = e.target && e.target.closest ? e.target.closest(PILE_SELECTOR) : null;
    if(pileEl) shakeOff(pileEl);
    if(!el) return;
  ...
  /* ── SNOW SETTLING ON IDLE BOXES ──────────────────────────
     While the screen sits open, snowballs quietly gather on the
     cards in view — mostly along the top edge where snow would
     really pile, with the odd flake settling mid-surface. Tap a
     card and it all shakes loose (see shakeOff above). */
  function accumulate(){
    if(!running || document.documentElement.dataset.theme !== THEME){ schedulePile(); return; }
    var vh = window.innerHeight;
    var cards = [];
    document.querySelectorAll(PILE_SELECTOR).forEach(function(c){
      var r = c.getBoundingClientRect();
      if(r.width > 60 && r.height > 24 && r.bottom > 0 && r.top < vh) cards.push(c);
    });
    if(cards.length){
      var n = _grndFxN(1, 2);
      for(var i=0;i<n;i++){
        var card = cards[Math.floor(Math.random()*cards.length)];
        if(card.querySelectorAll('.winter-snowpile').length >= PILE_CAP_PER_CARD) continue;
        var b = document.createElement('span');
        b.className = 'winter-snowpile';
        var onEdge = Math.random() < 0.7;
        var s = onEdge ? rand(3.5, 7.5) : rand(2.5, 4.5);
        b.style.width  = s + 'px';
        b.style.height = s + 'px';
        b.style.left   = rand(2, 95) + '%';
        b.style.top    = onEdge ? rand(0, 4) + 'px' : rand(10, 85) + '%';
        if(!onEdge) b.style.opacity = '0.75';
        card.appendChild(b);
        b.animate([
          { transform:'translateY(-8px) scale(0.4)', opacity:0 },
          { transform:'translateY(0) scale(1)',      opacity: onEdge ? 1 : 0.75 }
  ...
  function schedulePile(){
    ...
    pileTimer = setTimeout(accumulate, _grndFxIv(rand(2400, 4600)));
  }
  function clearPiles(){
    document.querySelectorAll('.winter-snowpile').forEach(function(b){
      if(b.parentNode) b.parentNode.removeChild(b);
    });
  }

  /* A slow wisp of freezing fog rolling low across the screen. */
  function spawnFog(){
    if(!running || document.documentElement.dataset.theme !== THEME){ scheduleFog(); return; }
    ensureOverlay();
    var dir = Math.random()<0.5 ? 1 : -1;
    var w = rand(window.innerWidth*0.4, window.innerWidth*0.7);
    var h = rand(50, 110);
    var y = rand(window.innerHeight*0.55, window.innerHeight*0.85);
    var startX = dir>0 ? -w : window.innerWidth;
    var endX = dir>0 ? window.innerWidth : -w;
    var dur = rand(26000, 40000);
    var fog = document.createElement('span');
    fog.className = 'winter-flake';
    fog.style.cssText = 'position:absolute; left:0; top:0; width:'+w+'px; height:'+h+'px; border-radius:50%; '+
      'background:radial-gradient(ellipse, rgba(198,224,244,0.16) 0%, rgba(198,224,244,0.06) 55%, rgba(198,224,244,0) 78%); '+
      'filter:blur(10px); transform:translate('+startX+'px,'+y+'px);';
    overlay.appendChild(fog);
    fog.animate([
      { transform:'translate('+startX+'px,'+y+'px)', opacity:0 },
      { opacity:1, offset:0.18 },
      { opacity:1, offset:0.8 },
      { transform:'translate('+endX+'px,'+(y-14)+'px)', opacity:0 }
    ], { duration:dur, easing:'linear', fill:'forwards' });
    gone(fog, dur+200);
    scheduleFog();
  }
  function scheduleFog(){
    ...
    fogTimer = setTimeout(spawnFog, rand(20000, 36000));
  }
  ...
  function start(){
    ...
    ensureOverlay(); clearOverlay();
    spawnSnow();
    fogTimer  = setTimeout(spawnFog, rand(9000, 16000));
    pileTimer = setTimeout(accumulate, rand(4000, 7000));
  }
  function stop(){
    running = false;
    clearTimeout(snowTimer); clearTimeout(fogTimer);
    clearTimeout(startTimer); clearTimeout(pileTimer);
    clearOverlay();
    clearPiles();
  }
  function sync(){ if(document.documentElement.dataset.theme === THEME) start(); else stop(); }
...
/* ══ SPRING — drifting morning clouds & floating petals ═══════ */
(function(){
  var THEME = 'spring';
  var overlay = null, running = false;
  var cloudTimer=null, petalTimer=null, startTimer=null;
  var grassTimer=null, sheenTimer=null;

  function rand(a,b){ return a + Math.random()*(b-a); }
  function ensureOverlay(){
    if(overlay) return overlay;
    overlay = document.getElementById('spring-fx-overlay');
    if(!overlay){
      overlay = document.createElement('div');
      overlay.id = 'spring-fx-overlay';
      overlay.setAttribute('aria-hidden','true');
      overlay.style.cssText = 'position:fixed; inset:0; overflow:hidden; pointer-events:none; z-index:9999;';
      document.body.appendChild(overlay);
    }
  ...
  /* Scene layer behind the content: clouds + wind sheen. */
  var sky = null;
  function ensureSky(){
    if(sky) return sky;
    sky = document.getElementById('spring-sky-overlay');
    if(!sky){
      sky = document.createElement('div');
      sky.id = 'spring-sky-overlay';
      sky.setAttribute('aria-hidden','true');
      sky.style.cssText = 'position:fixed; inset:0; overflow:hidden; pointer-events:none; z-index:1;';
      document.body.appendChild(sky);
    }
  ...
  /* A soft white cloud sailing slowly across the morning sky. */
  function spawnCloud(){
    if(!running || document.documentElement.dataset.theme !== THEME){ scheduleCloud(); return; }
    ensureOverlay();
    var dir = Math.random()<0.5 ? 1 : -1;
    var w = rand(120, 260);
    var h = w * rand(0.28, 0.38);
    var y = rand(window.innerHeight*0.04, window.innerHeight*0.30);
    var startX = dir>0 ? -w : window.innerWidth;
    var endX = dir>0 ? window.innerWidth : -w;
    var dur = rand(42000, 70000);
    var cloud = document.createElement('span');
    cloud.className = 'spring-fx';
    cloud.style.cssText = 'position:absolute; left:0; top:0; width:'+w+'px; height:'+h+'px; '+
      'background:'+
        'radial-gradient(38% 62% at 30% 62%, rgba(255,255,255,0.92) 0 58%, rgba(255,255,255,0) 78%),'+
        'radial-gradient(46% 78% at 52% 46%, rgba(255,255,255,0.96) 0 58%, rgba(255,255,255,0) 78%),'+
        'radial-gradient(34% 58% at 74% 60%, rgba(255,255,255,0.88) 0 58%, rgba(255,255,255,0) 78%); '+
      'filter:blur(2px); transform:translate('+startX+'px,'+y+'px);';
    ensureSky().appendChild(cloud);
    cloud.animate([
      { transform:'translate('+startX+'px,'+y+'px)', opacity:0 },
      ...
      { transform:'translate('+endX+'px,'+(y+rand(-10,10))+'px)', opacity:0 }
    ], { duration:dur, easing:'linear', fill:'forwards' });
    gone(cloud, dur+200);
    scheduleCloud();
  }
  function scheduleCloud(){
    if(!running || document.documentElement.dataset.theme !== THEME) return;
    clearTimeout(cloudTimer);
    cloudTimer = setTimeout(spawnCloud, rand(14000, 26000));
  }

  /* Tiny blossom petals floating by on the breeze. */
  function spawnPetals(){
    if(!running || document.documentElement.dataset.theme !== THEME){ schedulePetals(); return; }
    ensureOverlay();
    var count = _grndFxN(2, 2);
    var colors = ['rgba(255,214,228,0.9)','rgba(255,255,255,0.9)','rgba(255,232,150,0.85)'];
    for(var i=0;i<count;i++){
      var p = document.createElement('span');
      p.className = 'spring-fx';
      var s = rand(4, 8);
      var x = rand(window.innerWidth*0.05, window.innerWidth*0.95);
      var y = rand(window.innerHeight*0.15, window.innerHeight*0.6);
      var driftX = rand(50, 140) * (Math.random()<0.5?-1:1);
      var driftY = rand(60, 140);
      var rot = rand(120, 400) * (Math.random()<0.5?-1:1);
      var dur = rand(7000, 12000);
      p.style.cssText = 'position:absolute; left:0; top:0; width:'+s+'px; height:'+(s*0.7)+'px; '+
        'border-radius:60% 40% 55% 45%; background:'+colors[Math.floor(Math.random()*colors.length)]+'; '+
        'transform:translate('+x+'px,'+y+'px);';
      overlay.appendChild(p);
      p.animate([
        { transform:'translate('+x+'px,'+y+'px) rotate(0deg)', opacity:0 },
        { opacity:0.9, offset:0.12 },
        { transform:'translate('+(x+driftX*0.5)+'px,'+(y+driftY*0.4)+'px) rotate('+(rot*0.5)+'deg)', offset:0.5 },
        { transform:'translate('+(x+driftX)+'px,'+(y+driftY)+'px) rotate('+rot+'deg)', opacity:0 }
      ], { duration:dur, easing:'ease-in-out', fill:'forwards' });
      gone(p, dur+150);
    }
    schedulePetals();
  ...
  function schedulePetals(){
    ...
    petalTimer = setTimeout(spawnPetals, _grndFxIv(rand(4000, 8000)));
  }

  /* ── WIND THROUGH THE GRASS ────────────────────────────────
     A gust carries a handful of grass blades across the meadow
     band, tumbling and swaying as they go. */
  function spawnGrassGust(){
    if(!running || document.documentElement.dataset.theme !== THEME){ scheduleGrass(); return; }
    ensureOverlay();
    var dir = Math.random() < 0.5 ? 1 : -1;
    var count = _grndFxN(4, 3);
    var bandY = rand(window.innerHeight*0.64, window.innerHeight*0.94);
    var travel = rand(window.innerWidth*0.55, window.innerWidth*0.95);
    var baseDur = rand(4800, 7200);
    for(var i=0;i<count;i++){
      var blade = document.createElement('span');
      blade.className = 'spring-fx';
      var w = rand(3, 5);
      var len = rand(16, 32);
      var startX = dir > 0 ? rand(-50,-12) : window.innerWidth + rand(12,50);
      var y = bandY + rand(-30, 30);
      var bob = rand(12, 28);
      var rot0 = rand(-30, 30);
      var spin = rand(-46, 46);
      var op = rand(0.45, 0.75);
      var dur = baseDur * rand(0.85, 1.18);
      var delay = i * rand(90, 220);
      var endX = startX + dir*travel;
      var curve = rand(-20, 20);
      blade.style.cssText = 'position:absolute; left:0; top:0; width:'+w+'px; height:'+len+'px; '+
        'clip-path:polygon('+(50+curve*0.4)+'% 0%, 66% 30%, 60% 72%, 50% 100%, 40% 72%, 34% 30%); '+
        'background:linear-gradient(to top, rgba(58,116,30,0) 0%, rgba(78,140,40,0.92) 26%, rgba(108,168,58,0.96) 58%, rgba(150,200,86,0.96) 82%, rgba(216,240,150,0.98) 100%); '+
        'filter:blur(0.25px); '+
        'transform:translate('+startX+'px,'+y+'px) rotate('+rot0+'deg);';
      overlay.appendChild(blade);
      blade.animate([
        { transform:'translate('+startX+'px,'+y+'px) rotate('+rot0+'deg)', opacity:0 },
        { transform:'translate('+(startX+dir*travel*0.25)+'px,'+(y - bob*0.6)+'px) rotate('+(rot0+spin*0.3)+'deg)', opacity:op, offset:0.22 },
        { transform:'translate('+(startX+dir*travel*0.55)+'px,'+(y + bob*0.4)+'px) rotate('+(rot0+spin*0.6)+'deg)', opacity:op, offset:0.55 },
        { transform:'translate('+(startX+dir*travel*0.82)+'px,'+(y - bob*0.35)+'px) rotate('+(rot0+spin*0.85)+'deg)', opacity:op*0.8, offset:0.82 },
        { transform:'translate('+endX+'px,'+(y - bob)+'px) rotate('+(rot0+spin)+'deg)', opacity:0 }
      ], { duration:dur, delay:delay, easing:'ease-in-out', fill:'forwards' });
      setTimeout(function(el){ if(el && el.parentNode) el.parentNode.removeChild(el); }, dur+delay+250, blade);
    }
    scheduleGrass();
  ...
  function scheduleGrass(){
    if(!running || document.documentElement.dataset.theme !== THEME) return;
    clearTimeout(grassTimer);
    grassTimer = setTimeout(spawnGrassGust, _grndFxIv(rand(5500, 10500)));
  }

  /* A band of light rolling across the meadow — the visible shape
     of the wind moving through the grass. */
  function spawnSheen(){
    if(!running || document.documentElement.dataset.theme !== THEME){ scheduleSheen(); return; }
    ensureOverlay();
    var dir = Math.random() < 0.5 ? 1 : -1;
    var w = window.innerWidth * rand(0.28, 0.42);
    var top = window.innerHeight * 0.60;
    var h = window.innerHeight * 0.42;
    var startX = dir>0 ? -w*1.4 : window.innerWidth + w*0.4;
    var endX = dir>0 ? window.innerWidth + w*0.4 : -w*1.4;
    var dur = rand(4200, 6400);
    var sheen = document.createElement('span');
    sheen.className = 'spring-fx spring-sheen';
    sheen.style.cssText = 'position:absolute; left:0; top:'+top+'px; width:'+w+'px; height:'+h+'px; '+
      'background:linear-gradient('+(dir>0?100:80)+'deg, rgba(240,255,200,0) 0%, rgba(240,255,200,0.13) 45%, rgba(255,255,220,0.07) 60%, rgba(240,255,200,0) 100%); '+
      'filter:blur(6px); transform:translateX('+startX+'px) skewX('+(dir*-8)+'deg);';
    ensureSky().appendChild(sheen);
    sheen.animate([
      { transform:'translateX('+startX+'px) skewX('+(dir*-8)+'deg)', opacity:0 },
      { opacity:1, offset:0.25 },
      { opacity:1, offset:0.75 },
      { transform:'translateX('+endX+'px) skewX('+(dir*-8)+'deg)', opacity:0 }
    ], { duration:dur, easing:'ease-in-out', fill:'forwards' });
    setTimeout(function(el){ if(el && el.parentNode) el.parentNode.removeChild(el); }, dur+200, sheen);
    scheduleSheen();
  }
  function scheduleSheen(){
    if(!running || document.documentElement.dataset.theme !== THEME) return;
    clearTimeout(sheenTimer);
    sheenTimer = setTimeout(spawnSheen, _grndFxIv(rand(7000, 13000)));
  }
  ...
  function start(){
    ...
    ensureOverlay(); clearOverlay();
    spawnCloud();
    petalTimer = setTimeout(spawnPetals, rand(2500, 5000));
    grassTimer = setTimeout(spawnGrassGust, rand(2000, 4500));
    sheenTimer = setTimeout(spawnSheen, rand(3000, 6000));
  }
  function stop(){
    running = false;
    clearTimeout(cloudTimer); clearTimeout(petalTimer); clearTimeout(startTimer);
    clearTimeout(grassTimer); clearTimeout(sheenTimer);
    clearOverlay();
  }
  function sync(){ if(document.documentElement.dataset.theme === THEME) start(); else stop(); }
  if(document.readyState === 'loading') document.addEventListener('DOMContentLoaded', sync);
...
/* ══ AUTUMN — tumbling leaves & sweeping wind gusts ═══════════ */
(function(){
  var THEME = 'autumn';
  var overlay = null, running = false;
  var leafTimer=null, gustTimer=null, startTimer=null, cloudTimer=null;
  var LEAF_COLORS = [
    'linear-gradient(135deg, #d98a2b, #b8641a)',
    'linear-gradient(135deg, #c8741e, #9c4f14)',
    'linear-gradient(135deg, #b8942a, #8f6d1c)',
    'linear-gradient(135deg, #a8502a, #7e3418)',
    'linear-gradient(135deg, #c9a53a, #a67c24)'
  ];

  function rand(a,b){ return a + Math.random()*(b-a); }
  function ensureOverlay(){
    if(overlay) return overlay;
    overlay = document.getElementById('autumn-fx-overlay');
    if(!overlay){
      overlay = document.createElement('div');
      overlay.id = 'autumn-fx-overlay';
      overlay.setAttribute('aria-hidden','true');
      overlay.style.cssText = 'position:fixed; inset:0; overflow:hidden; pointer-events:none; z-index:9999;';
      document.body.appendChild(overlay);
    }
  ...
  /* Scene layer behind the content: the crawling overcast. */
  var sky = null;
  function ensureSky(){
    if(sky) return sky;
    sky = document.getElementById('autumn-sky-overlay');
    if(!sky){
      sky = document.createElement('div');
      sky.id = 'autumn-sky-overlay';
      sky.setAttribute('aria-hidden','true');
      sky.style.cssText = 'position:fixed; inset:0; overflow:hidden; pointer-events:none; z-index:1;';
      document.body.appendChild(sky);
    }
  ...
  function makeLeaf(s){
    ...
    leaf.className = 'autumn-fx';
    /* leaf silhouette: two opposite rounded corners */
    leaf.style.cssText = 'position:absolute; left:0; top:0; width:'+s+'px; height:'+s+'px; '+
      'border-radius: 0 62% 0 62%; '+
      'background:'+LEAF_COLORS[Math.floor(Math.random()*LEAF_COLORS.length)]+';';
    return leaf;
  }

  /* Leaves detaching high up and tumbling down the whole screen. */
  function spawnLeaves(){
    ...
    for(var i=0;i<count;i++){
      ...
      var x = rand(-20, window.innerWidth+20);
      var y = -rand(20, 60);
      var fallTo = window.innerHeight + 30;
      var sway = rand(60, 160) * (Math.random()<0.5?-1:1);
      var rot = rand(240, 720) * (Math.random()<0.5?-1:1);
      var dur = rand(8000, 14000);
      var op = rand(0.65, 0.95);
      var leaf = makeLeaf(s);
      ...
      leaf.animate([
        { transform:'translate('+x+'px,'+y+'px) rotate(0deg)', opacity:0 },
        { opacity:op, offset:0.06 },
        { transform:'translate('+(x+sway)+'px,'+(y+(fallTo-y)*0.35)+'px) rotate('+(rot*0.35)+'deg)', offset:0.35 },
        { transform:'translate('+(x-sway*0.5)+'px,'+(y+(fallTo-y)*0.7)+'px) rotate('+(rot*0.7)+'deg)', opacity:op, offset:0.7 },
        { transform:'translate('+(x+sway*0.6)+'px,'+fallTo+'px) rotate('+rot+'deg)', opacity:0 }
      ], { duration:dur, easing:'ease-in-out', fill:'forwards' });
      gone(leaf, dur+200);
    }
    scheduleLeaves();
  ...
  function scheduleLeaves(){
    ...
    leafTimer = setTimeout(spawnLeaves, _grndFxIv(rand(2200, 4200)));
  }

  /* A wind gust sweeping a handful of leaves sideways across the
     field band — the same energy as the sidequests grass gusts. */
  function spawnGust(){
    if(!running || document.documentElement.dataset.theme !== THEME){ scheduleGust(); return; }
    ensureOverlay();
    var dir = Math.random()<0.5 ? 1 : -1;
    var count = _grndFxN(4, 3);
    var bandY = rand(window.innerHeight*0.45, window.innerHeight*0.85);
    var travel = rand(window.innerWidth*0.6, window.innerWidth*1.0);
    var baseDur = rand(4200, 6600);
    for(var i=0;i<count;i++){
      var s = rand(7, 13);
      var startX = dir>0 ? rand(-60,-14) : window.innerWidth + rand(14,60);
      var y = bandY + rand(-50, 50);
      var bob = rand(20, 55);
      var rot = rand(200, 560) * dir;
      var op = rand(0.6, 0.9);
      var dur = baseDur * rand(0.85, 1.2);
      var delay = i * rand(80, 220);
      var endX = startX + dir*travel;
      var leaf = makeLeaf(s);
      leaf.style.transform = 'translate('+startX+'px,'+y+'px)';
      overlay.appendChild(leaf);
      leaf.animate([
        { transform:'translate('+startX+'px,'+y+'px) rotate(0deg)', opacity:0 },
        { transform:'translate('+(startX+dir*travel*0.25)+'px,'+(y-bob*0.7)+'px) rotate('+(rot*0.3)+'deg)', opacity:op, offset:0.25 },
        { transform:'translate('+(startX+dir*travel*0.55)+'px,'+(y+bob*0.4)+'px) rotate('+(rot*0.6)+'deg)', opacity:op, offset:0.55 },
        { transform:'translate('+endX+'px,'+(y-bob*0.5)+'px) rotate('+rot+'deg)', opacity:0 }
      ], { duration:dur, delay:delay, easing:'ease-in-out', fill:'forwards' });
      gone(leaf, dur+delay+250);
    }
    scheduleGust();
  }
  function scheduleGust(){
    ...
    gustTimer = setTimeout(spawnGust, rand(9000, 16000));
  }

  /* Warm-grey autumn clouds crawling across the golden sky —
     underside caught by the low harvest light. */
  function spawnCloud(){
    if(!running || document.documentElement.dataset.theme !== THEME){ scheduleCloud(); return; }
    ensureOverlay();
    var dir = Math.random()<0.5 ? 1 : -1;
    var w = rand(160, 340);
    var h = w * rand(0.26, 0.36);
    var y = rand(window.innerHeight*0.02, window.innerHeight*0.26);
    var startX = dir>0 ? -w : window.innerWidth;
    var endX = dir>0 ? window.innerWidth : -w;
    var dur = rand(36000, 60000);
    var cloud = document.createElement('span');
    cloud.className = 'autumn-fx';
    cloud.style.cssText = 'position:absolute; left:0; top:0; width:'+w+'px; height:'+h+'px; '+
      'background:'+
        'radial-gradient(38% 60% at 30% 58%, rgba(122,114,96,0.60) 0 58%, rgba(122,114,96,0) 78%),'+
        'radial-gradient(46% 76% at 52% 42%, rgba(134,124,104,0.62) 0 58%, rgba(134,124,104,0) 78%),'+
        'radial-gradient(34% 56% at 74% 58%, rgba(126,116,98,0.55) 0 58%, rgba(126,116,98,0) 78%),'+
        'radial-gradient(60% 30% at 50% 84%, rgba(242,216,150,0.28) 0 60%, rgba(242,216,150,0) 80%); '+
      'filter:blur(3px); transform:translate('+startX+'px,'+y+'px);';
    ensureSky().appendChild(cloud);
    cloud.animate([
      { transform:'translate('+startX+'px,'+y+'px)', opacity:0 },
      { opacity:0.92, offset:0.07 },
  ...
  function scheduleCloud(){
    if(!running || document.documentElement.dataset.theme !== THEME) return;
    clearTimeout(cloudTimer);
    cloudTimer = setTimeout(spawnCloud, _grndFxIv(rand(9000, 18000)));
  }
  ...
  function start(){
    ...
    ensureOverlay(); clearOverlay();
    spawnLeaves();
    gustTimer  = setTimeout(spawnGust, rand(4000, 8000));
    cloudTimer = setTimeout(spawnCloud, rand(1500, 4000));
  }
  function stop(){
    running = false;
    clearTimeout(leafTimer); clearTimeout(gustTimer);
    clearTimeout(startTimer); clearTimeout(cloudTimer);
    clearOverlay();
  }
  function sync(){ if(document.documentElement.dataset.theme === THEME) start(); else stop(); }
  if(document.readyState === 'loading') document.addEventListener('DOMContentLoaded', sync);
...
/* ===== SCROLL PERFORMANCE: pause background/shimmer animations while scrolling =====
   Toggles html.is-scrolling (see themes.css "SCROLL PERFORMANCE" block), which
   pauses the always-on per-theme background sheen and the glass-style card
   shimmer for the brief duration of a scroll. Purely a perf fix -- no visual
   change to the animations themselves, they just resume right after scrolling
   stops. Capture-phase + passive listener on window catches scroll events from
   any scrollable container (modals, panels), not just the page itself. */
(function(){
  ...
  window.addEventListener('scroll', function(){
    ...
    scrollTimer = setTimeout(function(){ root.classList.remove('is-scrolling'); }, 150);

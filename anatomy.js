...
/* SVG area */
.svg-wrap{flex:1;display:flex;align-items:center;justify-content:center;padding:14px 10px;position:relative;z-index:1;overflow:hidden}
#bodySvg{width:100%;height:100%;max-height:calc(100% - 0px);display:block}
...
/* Status bar */
.status-bar{padding:9px 12px;border-top:1px solid var(--border);display:flex;align-items:center;justify-content:space-between;flex-shrink:0;background:var(--bg2);position:relative;z-index:2}
.status-text{font-family:var(--mono);font-size:0.57rem;color:var(--text3);letter-spacing:0.06em;text-transform:uppercase}
.status-hint{font-family:var(--mono);font-size:0.5rem;color:var(--text3);letter-spacing:0.06em;opacity:0.5}
...
.sub-info-banner.visible{display:block}
.sub-info-legend{
  display:flex;
  flex-wrap:wrap;
  gap:6px;
...
/* ── INFO PANEL ── */
.info-panel{flex:1;overflow-y:auto;overflow-x:hidden;background:var(--bg2);display:flex;flex-direction:column;position:relative;min-width:0}
@media(max-width:767px){.info-panel{border-top:1px solid var(--border);min-height:300px}}
...
`;
  ...
  <div class="viewer-panel">
    ...
    <div class="view-controls">
      ...
      <div id="sidePicker">
        <button class="side-btn active" id="sideAll" onclick="setSide('both')" title="Highlight both sides">⊕</button>
    ...
    <div class="svg-wrap" id="svgWrap">
      <svg id="bodySvg" viewBox="0 0 200 520" xmlns="http://www.w3.org/2000/svg">
        <defs>
    ...
    <div class="status-bar">
      <div class="status-text" id="statusText">HOVER A REGION TO INSPECT</div>
      <div style="display:flex;align-items:center;gap:10px">
        <div class="status-hint" id="statusHint">CLICK TO LOCK · ESC TO CLEAR</div>
        <button class="reset-btn" id="resetBtn" onclick="clearSelection()">&#8635; RESET</button>
      </div>
    </div>
...
function onEnter(e, m) {
  ...
  if(!selectedKey) statusEl.textContent = (d ? d.name.toUpperCase() : m.key) + ' — CLICK TO INSPECT';
}
...
function onLeave(m) {
  ...
  if(!selectedKey) statusEl.textContent = 'HOVER A REGION TO INSPECT';
}
...
function selectMuscle(m) {
  ...
  const d = MUSCLES[m.key];
  statusEl.textContent = (d ? d.name.toUpperCase() : m.key) + ' — SELECTED';
  statusHint.textContent = isMobile ? 'TAP SUB-MUSCLE · ESC TO CLEAR' : 'HOVER SUB-MUSCLES · ESC TO CLEAR';
}
...
function clearSelection() {
  ...
  zoomReset();
  hideInfo();
  statusEl.textContent = 'HOVER A REGION TO INSPECT';
  statusHint.textContent = 'CLICK TO LOCK · ESC TO CLEAR';
}
...
function onSubClick(s, pal, el) {
  ...
  statusHint.textContent = 'TAP AGAIN TO ZOOM OUT · ESC TO CLEAR';

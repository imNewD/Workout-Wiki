...
@media(max-width:767px){.viewer-panel{width:100%;max-width:100%;height:var(--viewer-h,75vh);height:var(--viewer-h,clamp(55svh,75svh,82svh))}}
...
@media(max-width:767px){.resize-handle{display:flex}}
.resize-handle-bar{width:36px;height:4px;border-radius:2px;background:var(--border2);transition:background .15s}
...
.layer-chip{font-family:var(--mono);font-size:clamp(0.44rem,1.3vw,0.5rem);letter-spacing:0.08em;text-transform:uppercase;padding:clamp(3px,1vw,4px) clamp(6px,2vw,9px);border-radius:5px;border:1px solid var(--border2);color:var(--text3);transition:all var(--trans)}
.layer-chip.on{border-color:var(--accent);color:var(--accent);background:rgba(230,57,70,0.08)}
.layer-chip.locked{opacity:0.35;cursor:not-allowed}
.layer-chip.locked::after{content:" ◌";font-size:0.44rem}

/* ── LAYER DROPDOWNS ── */
.layer-dd{position:relative;display:inline-block}
.layer-dd-panel{display:none;position:absolute;top:calc(100% + 5px);left:0;min-width:130px;background:var(--bg2);border:1px solid var(--border2);border-radius:7px;padding:6px 4px;z-index:200;box-shadow:0 6px 20px rgba(0,0,0,.4);animation:slideUp .15s ease}
.layer-dd.open .layer-dd-panel{display:block}
.dd-row{display:flex;align-items:center;gap:8px;padding:5px 9px;border-radius:5px;cursor:pointer;font-family:var(--mono);font-size:0.5rem;letter-spacing:0.08em;text-transform:uppercase;color:var(--text2);transition:background .12s}
.dd-row:hover{background:var(--bg3)}
.dd-row input[type=checkbox]{accent-color:var(--accent);width:12px;height:12px;cursor:pointer;flex-shrink:0}
.dd-locked{opacity:0.4;cursor:not-allowed;pointer-events:none}
.dd-v2{font-family:var(--mono);font-size:0.42rem;color:var(--text3);background:var(--bg3);border:1px solid var(--border2);border-radius:3px;padding:1px 4px;margin-left:auto}
.dd-placeholder{font-family:var(--mono);font-size:0.48rem;letter-spacing:0.1em;text-transform:uppercase;color:var(--text3);padding:8px 10px;text-align:center}
...
.muscle-path{
  ...
  filter:drop-shadow(0 1px 6px rgba(0,0,0,0.72)) drop-shadow(0 0 2px rgba(215,52,68,0.14));
}
.muscle-path:hover,.muscle-path.hovered{fill:var(--muscle-hover);stroke:var(--muscle-stroke-h);stroke-width:1.15;filter:drop-shadow(0 3px 12px rgba(220,55,70,0.58)) drop-shadow(0 1px 6px rgba(0,0,0,0.68)) drop-shadow(0 0 4px rgba(255,140,155,0.22))}
.muscle-path.selected{fill:var(--muscle-active);stroke:rgba(238,68,82,0.99);stroke-width:1.25;animation:musclePulse 2.2s ease-in-out infinite}
...
@keyframes musclePulse{
  0%,100%{filter:drop-shadow(0 0 5px rgba(230,57,70,.58)) drop-shadow(0 1px 6px rgba(0,0,0,0.58)) drop-shadow(0 0 1px rgba(255,190,200,0.18))}
  50%{filter:drop-shadow(0 0 18px rgba(230,57,70,1.0)) drop-shadow(0 0 10px rgba(255,145,165,.55)) drop-shadow(0 0 4px rgba(255,220,225,0.30)) drop-shadow(0 1px 6px rgba(0,0,0,0.58))}
}
...
/* ── SEARCH BAR ── */
.anatomy-search-wrap{position:relative;padding:8px 12px;border-bottom:1px solid var(--border);background:var(--bg);flex-shrink:0}
.anatomy-search-input{width:100%;box-sizing:border-box;background:var(--bg3);border:1px solid var(--border2);border-radius:7px;padding:7px 32px 7px 30px;font-family:var(--mono);font-size:0.6rem;letter-spacing:0.05em;color:var(--text);outline:none;transition:border-color .18s,box-shadow .18s}
.anatomy-search-input:focus{border-color:var(--accent);box-shadow:0 0 0 2px rgba(230,57,70,0.14)}
.anatomy-search-input::placeholder{color:var(--text3);opacity:0.7}
.anatomy-search-icon{position:absolute;left:20px;top:50%;transform:translateY(-50%);font-size:0.64rem;color:var(--text3);pointer-events:none}
.anatomy-search-clear{position:absolute;right:20px;top:50%;transform:translateY(-50%);font-family:var(--mono);font-size:0.52rem;color:var(--text3);cursor:pointer;padding:2px 5px;border-radius:3px;transition:color .12s;display:none}
.anatomy-search-clear:hover{color:var(--accent)}
.anatomy-search-clear.show{display:block}
.anatomy-search-results{position:absolute;left:12px;right:12px;top:calc(100% - 8px);z-index:50;background:var(--bg2);border:1px solid var(--border2);border-radius:8px;box-shadow:0 8px 28px rgba(0,0,0,0.45);max-height:260px;overflow-y:auto;display:none}
.anatomy-search-results.open{display:block}
.anatomy-search-result{padding:9px 14px;font-family:var(--mono);font-size:0.58rem;letter-spacing:0.06em;text-transform:uppercase;color:var(--text2);cursor:pointer;transition:background .1s,color .1s;border-bottom:1px solid var(--border);display:flex;align-items:center;gap:10px}
.anatomy-search-result:last-child{border-bottom:none}
.anatomy-search-result:hover,.anatomy-search-result.focused{background:var(--bg3);color:var(--accent)}
.anatomy-search-result-name{flex:1}
.anatomy-search-result-region{font-size:0.48rem;color:var(--text3);letter-spacing:0.08em;flex-shrink:0}
.anatomy-search-result-sci{font-family:var(--sans);font-size:0.52rem;letter-spacing:0;text-transform:none;color:var(--text3);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:120px}
.anatomy-search-empty{padding:14px;text-align:center;font-family:var(--mono);font-size:0.56rem;color:var(--text3)}
.anatomy-search-kbd{position:absolute;right:20px;top:50%;transform:translateY(-50%);font-family:var(--mono);font-size:0.46rem;color:var(--text3);background:var(--bg3);border:1px solid var(--border2);border-radius:3px;padding:1px 5px;pointer-events:none}

/* ── EXERCISE MODE PANEL ── */
.exercise-mode-bar{display:none;padding:8px 12px;background:rgba(230,57,70,0.06);border-bottom:1px solid rgba(230,57,70,0.2);flex-shrink:0;gap:6px;align-items:center;flex-wrap:wrap}
.exercise-mode-bar.active{display:flex}
.exercise-mode-lbl{font-family:var(--mono);font-size:0.48rem;letter-spacing:0.1em;text-transform:uppercase;color:var(--accent);flex-shrink:0}
.exercise-mode-muscles{display:flex;flex-wrap:wrap;gap:4px;flex:1}
.exercise-mode-chip{font-family:var(--mono);font-size:0.48rem;letter-spacing:0.07em;text-transform:uppercase;padding:3px 8px;border-radius:5px;cursor:pointer;transition:all .15s;border:1px solid transparent}
.exercise-mode-chip.primary{background:rgba(230,57,70,0.16);color:var(--accent);border-color:rgba(230,57,70,0.4)}
.exercise-mode-chip.secondary{background:rgba(247,162,97,0.14);color:#f4a261;border-color:rgba(247,162,97,0.35)}
.exercise-mode-chip.stabilizer{background:rgba(52,211,153,0.12);color:#34d399;border-color:rgba(52,211,153,0.3)}
.exercise-mode-chip:hover{filter:brightness(1.2)}
.exercise-mode-close{font-family:var(--mono);font-size:0.48rem;color:var(--text3);padding:3px 8px;border:1px solid var(--border2);border-radius:4px;cursor:pointer;transition:all .15s;flex-shrink:0}
.exercise-mode-close:hover{color:var(--accent);border-color:rgba(230,57,70,0.4)}

/* ── RECENTLY VIEWED ── */
.recently-viewed{padding:10px 12px 6px;border-bottom:1px solid var(--border);flex-shrink:0;display:none}
.recently-viewed.has-items{display:block}
.recently-viewed-lbl{font-family:var(--mono);font-size:0.44rem;letter-spacing:0.13em;text-transform:uppercase;color:var(--text3);margin-bottom:6px}
.recently-viewed-chips{display:flex;flex-wrap:wrap;gap:4px}
.rv-chip{font-family:var(--mono);font-size:0.48rem;letter-spacing:0.07em;text-transform:uppercase;padding:4px 9px;border-radius:5px;border:1px solid var(--border2);color:var(--text3);cursor:pointer;transition:all .14s;background:var(--bg3)}
.rv-chip:hover{border-color:var(--accent);color:var(--accent);background:rgba(230,57,70,0.07)}
...
[data-anatomy-lite="1"] .scanlines,
[data-anatomy-lite="1"] .muscle-overlay{display:none!important}
...
[data-anatomy-lite="1"] .sub-muscle.sub-selected{filter:none!important;animation:none!important}
...
.filter-panel{display:none;flex-direction:column;gap:0;background:var(--bg2);border:1px solid var(--border2);border-radius:9px;animation:slideUp .18s ease;flex-shrink:0;overflow:hidden}
...
.fp-section-locked{opacity:0.45;pointer-events:none}
...
.fp-v2-badge{font-family:var(--mono);font-size:0.4rem;letter-spacing:0.1em;color:var(--text3);padding:2px 6px;border:1px solid var(--border2);border-radius:3px;background:var(--bg3)}
/* Chips */
...
/* Bone chips — gold */
.fp-chip.fp-bone:hover:not(:disabled){border-color:#d4c58a;color:#d4c58a;background:rgba(212,197,138,0.08)}
.fp-chip.fp-bone.active{border-color:#d4c58a;color:#d4c58a;background:rgba(212,197,138,0.12);box-shadow:inset 0 0 0 1px rgba(212,197,138,0.22)}
/* Tendon chips — teal */
.fp-chip.fp-tendon:hover:not(:disabled){border-color:#22d3ee;color:#22d3ee;background:rgba(34,211,238,0.07)}
.fp-chip.fp-tendon.active{border-color:#22d3ee;color:#22d3ee;background:rgba(34,211,238,0.11);box-shadow:inset 0 0 0 1px rgba(34,211,238,0.2)}
/* Nerve chips — green */
.fp-chip.fp-nerve:hover:not(:disabled){border-color:#34d399;color:#34d399;background:rgba(52,211,153,0.07)}
.fp-chip.fp-nerve.active{border-color:#34d399;color:#34d399;background:rgba(52,211,153,0.11);box-shadow:inset 0 0 0 1px rgba(52,211,153,0.2)}
/* Fiber sub-section */
    ...
    <div class="info-idle" id="infoIdle">
      <div class="msearch-wrap">
        ...
        <div class="msearch-filters" id="msearchFilters">
          ...
          <button class="mfchip mfchip-locked" title="Coming in V2">TENDONS</button>
          <button class="mfchip mfchip-locked" title="Coming in V2">NERVES</button>
        ...
        <div class="filter-panel" id="filterPanel">
          ...
          <!-- ── BONES (placeholder — V2) ─────────────────── -->
          <div class="fp-section fp-section-locked">
            <div class="fp-section-hdr">
              <span class="fp-section-name">BONES</span>
              <span class="fp-v2-badge">V2</span>
            </div>
            <div class="fp-chips">
              <button class="fp-chip fp-bone" disabled>SKULL</button>
              <button class="fp-chip fp-bone" disabled>SPINE</button>
              <button class="fp-chip fp-bone" disabled>CHEST</button>
              <button class="fp-chip fp-bone" disabled>SHOULDER</button>
              <button class="fp-chip fp-bone" disabled>ARMS</button>
              <button class="fp-chip fp-bone" disabled>ELBOW</button>
              <button class="fp-chip fp-bone" disabled>PELVIS</button>
              <button class="fp-chip fp-bone" disabled>LEGS</button>
              <button class="fp-chip fp-bone" disabled>KNEE</button>
              <button class="fp-chip fp-bone" disabled>ANKLE</button>
            </div>
          </div>

          <!-- ── NERVES (placeholder — V2) ────────────────── -->
          <div class="fp-section fp-section-locked">
            <div class="fp-section-hdr">
              <span class="fp-section-name">NERVES</span>
              <span class="fp-v2-badge">V2</span>
            </div>
            <div class="fp-chips">
              <button class="fp-chip fp-nerve" disabled>CERVICAL</button>
              <button class="fp-chip fp-nerve" disabled>BRACHIAL</button>
              <button class="fp-chip fp-nerve" disabled>THORACIC</button>
              <button class="fp-chip fp-nerve" disabled>ABS</button>
              <button class="fp-chip fp-nerve" disabled>LUMBAR</button>
              <button class="fp-chip fp-nerve" disabled>SCIATIC</button>
              <button class="fp-chip fp-nerve" disabled>FEMORAL</button>
            </div>
          </div>

          <!-- ── TENDONS (placeholder — V2) ───────────────── -->
          <div class="fp-section fp-section-locked">
            <div class="fp-section-hdr">
              <span class="fp-section-name">TENDONS</span>
              <span class="fp-v2-badge">V2</span>
            </div>
            <div class="fp-chips">
              <button class="fp-chip fp-tendon" disabled>SHOULDER</button>
              <button class="fp-chip fp-tendon" disabled>CHEST</button>
              <button class="fp-chip fp-tendon" disabled>ELBOW</button>
              <button class="fp-chip fp-tendon" disabled>WRIST</button>
              <button class="fp-chip fp-tendon" disabled>CORE</button>
              <button class="fp-chip fp-tendon" disabled>HIP</button>
              <button class="fp-chip fp-tendon" disabled>KNEE</button>
              <button class="fp-chip fp-tendon" disabled>ACHILLES</button>
            </div>
          </div>
...
const FRONT = [
  ...
  {id:'bic-L',key:'biceps',label:'BIC',cx:40,cy:158,
   d:'M28,136 Q20,158 22,180 Q26,198 46,202 Q60,198 66,180 Q68,160 60,138 Q50,130 36,132 Z'},
  {id:'bic-R',key:'biceps',label:'BIC',cx:160,cy:158,
   d:'M172,136 Q180,158 178,180 Q174,198 154,202 Q140,198 134,180 Q132,160 140,138 Q150,130 164,132 Z'},
  /* Forearms — tapered tube */
  {id:'fore-L',key:'forearms',label:'FORE',cx:36,cy:238,
   d:'M28,194 Q15,220 16,256 Q20,278 34,290 Q46,298 56,288 Q59,268 58,244 Q56,214 46,194 Z'},
  {id:'fore-R',key:'forearms',label:'FORE',cx:164,cy:238,
   d:'M172,194 Q185,220 184,256 Q180,278 166,290 Q154,298 144,288 Q141,268 142,244 Q144,214 154,194 Z'},
  /* Abs — rectus abdominis column */
  {id:'abs',key:'abs',label:'ABS',cx:100,cy:174,
  ...
  {id:'obl-L',key:'obliques',label:'OBL',cx:66,cy:172,
   d:'M64,126 Q54,148 54,178 Q56,210 70,226 Q78,232 84,224 Q82,192 82,164 Q82,140 78,128 Z'},
  {id:'obl-R',key:'obliques',label:'OBL',cx:134,cy:172,
  ...
  {id:'quad-L',key:'quadriceps',label:'QUAD',cx:73,cy:330,
   d:'M58,270 Q46,294 48,342 Q52,378 66,398 Q80,406 96,398 Q100,378 100,338 Q98,290 88,270 Z'},
  {id:'quad-R',key:'quadriceps',label:'QUAD',cx:127,cy:330,
  ...
  {id:'serr-L',key:'serratus',label:'SERR',cx:56,cy:160,
   d:'M60,124 Q48,140 48,162 Q50,184 62,198 Q70,202 76,194 Q74,172 68,150 Q64,134 60,124 Z'},
  {id:'serr-R',key:'serratus',label:'SERR',cx:144,cy:160,
  ...
  {id:'tib-L',key:'tibialis',label:'TIB',cx:74,cy:435,
   d:'M64,398 Q57,422 60,458 Q65,480 78,486 Q90,484 94,468 Q95,440 92,412 Q86,396 72,394 Z'},
  {id:'tib-R',key:'tibialis',label:'TIB',cx:126,cy:435,
  ...
  {id:'intcost-L',key:'intercostals',label:'INTCOST',cx:66,cy:174,
   d:'M66,142 Q58,158 58,176 Q58,198 64,210 Q72,214 78,208 Q80,188 80,168 Q80,150 74,140 Z'},
  {id:'intcost-R',key:'intercostals',label:'INTCOST',cx:134,cy:174,
  ...
  {id:'peron-L',key:'peroneals',label:'PERON',cx:90,cy:436,
   d:'M88,398 Q83,422 83,446 Q85,468 91,478 Q98,474 100,462 Q100,440 98,416 Q96,400 88,398 Z'},
  {id:'peron-R',key:'peroneals',label:'PERON',cx:110,cy:436,
...
const BACK = [
  ...
  /* Triceps — full posterior arm */
  {id:'tri-L',key:'triceps',label:'TRI',cx:38,cy:166,
   d:'M24,138 Q14,162 16,186 Q20,204 42,210 Q58,206 64,188 Q66,166 56,140 Q44,132 28,136 Z'},
  {id:'tri-R',key:'triceps',label:'TRI',cx:162,cy:166,
   d:'M176,138 Q186,162 184,186 Q180,204 158,210 Q142,206 136,188 Q134,166 144,140 Q156,132 172,136 Z'},
  ...
  {id:'lback',key:'lowerback',label:'ERECTORS',cx:100,cy:200,
   d:'M84,148 Q76,174 76,206 Q78,238 88,256 Q94,264 100,264 Q106,264 112,256 Q122,238 124,206 Q124,174 116,148 Q110,142 100,140 Q90,142 84,148 Z'},
  /* Rhomboids — between medial scapular borders and spine */
  {id:'rhomb-L',key:'rhomboids',label:'RHOMB',cx:82,cy:124,
   d:'M100,106 Q88,108 80,116 Q74,126 76,138 Q80,148 90,152 Q98,150 100,136 Q100,120 100,106 Z'},
  {id:'rhomb-R',key:'rhomboids',label:'RHOMB',cx:118,cy:124,
   d:'M100,106 Q112,108 120,116 Q126,126 124,138 Q120,148 110,152 Q102,150 100,136 Q100,120 100,106 Z'},
  /* Glutes — large rounded mass covering posterior hip */
  {id:'glut-L',key:'glutes',label:'GLUTE',cx:72,cy:308,
   d:'M52,266 Q40,290 44,322 Q50,354 74,364 Q94,366 102,346 Q104,318 96,288 Q88,266 70,260 Q58,262 52,266 Z'},
  {id:'glut-R',key:'glutes',label:'GLUTE',cx:128,cy:308,
   d:'M148,266 Q160,290 156,322 Q150,354 126,364 Q106,366 98,346 Q96,318 104,288 Q112,266 130,260 Q142,262 148,266 Z'},
  /* Hamstrings — posterior thigh from glute fold to knee */
  {id:'ham-L',key:'hamstrings',label:'HAM',cx:73,cy:398,
   d:'M56,360 Q46,386 50,418 Q56,444 76,450 Q96,448 102,430 Q104,402 100,372 Q94,350 76,354 Q64,356 56,360 Z'},
  {id:'ham-R',key:'hamstrings',label:'HAM',cx:127,cy:398,
   d:'M144,360 Q154,386 150,418 Q144,444 124,450 Q104,448 98,430 Q96,402 100,372 Q106,350 124,354 Q136,356 144,360 Z'},
  /* Calves — gastrocnemius diamond, two heads */
  {id:'calf-L',key:'calves',label:'CALF',cx:75,cy:464,
   d:'M58,450 Q50,470 54,488 Q62,508 80,512 Q96,510 101,492 Q103,468 98,446 Q90,432 76,436 Q66,440 58,450 Z'},
  {id:'calf-R',key:'calves',label:'CALF',cx:125,cy:464,
   d:'M142,450 Q150,470 146,488 Q138,508 120,512 Q104,510 99,492 Q97,468 102,446 Q110,432 124,436 Q134,440 142,450 Z'},
  /* Rotator Cuff — infraspinatus over lower scapular blade */
  {id:'rcuff-L',key:'rotatorcuff',label:'R.CUFF',cx:58,cy:130,
   d:'M50,110 Q40,124 42,140 Q46,152 60,156 Q72,152 76,138 Q74,122 64,110 Q58,106 50,110 Z'},
  {id:'rcuff-R',key:'rotatorcuff',label:'R.CUFF',cx:142,cy:130,
   d:'M150,110 Q160,124 158,140 Q154,152 140,156 Q128,152 124,138 Q126,122 136,110 Q142,106 150,110 Z'},
...
let currentVB      = {x:19,y:0,w:162,h:520};
const PAN_LIMIT     = 200;
...
function _haptic(ms) {
  try {
    ...
    // iOS AudioContext workaround — short click feel
...
function showDetail(key) {
  ...
  document.getElementById('subGroupLbl').textContent = muscleName + ' — SUBGROUPS';
  ...
  filtered.forEach((s, idx) => {
    ...
    const lbl = mkEl('text', {
      ...
      style: `opacity:0; fill:${pal.light}`,
    });
...
function showInfo(key) {
  ...
  if(iInjWrap) {
    if(d.injuries && d.injuries.length) {
      ...
      document.getElementById('iInjCards').innerHTML = d.injuries.map(inj => `
  ...
  if(iStrWrap) {
    if(d.stretches && d.stretches.length) {
      ...
      document.getElementById('iStrCards').innerHTML = d.stretches.map(s => `
...
function onBoneEnter(r) {
  ...
  if(isMobile) return;  // touch: tooltip gets stuck (mouseleave unreliable)
  getBoneEls(r.id).forEach(el => el.classList.add('bone-hovered'));
  getBoneLbls(r.id).forEach(l  => l.classList.add('bone-label-active'));
...
function onBoneLeave(r) {
  if(selectedBoneKey !== r.key) {
    getBoneEls(r.id).forEach(el => el.classList.remove('bone-hovered'));
    getBoneLbls(r.id).forEach(l  => l.classList.remove('bone-label-active'));
  }
  tooltip.classList.remove('show');
}
...
function getBoneEls(id) {
  // Match all regions with same key
  const key = (boneRegionLayer.querySelector(`[data-boneid="${id}"]`) || {dataset:{}}).dataset.bonekey;
  if(!key) return [];
  return [...boneRegionLayer.querySelectorAll(`.bone-region-path[data-bonekey="${key}"]`)];
}
function getBoneLbls(id) {
  const key = (boneRegionLayer.querySelector(`[data-boneid="${id}"]`) || {dataset:{}}).dataset.bonekey;
  if(!key) return [];
  return [...boneLabelLayer.querySelectorAll(`text`)].filter(t => {
    const el = boneRegionLayer.querySelector(`[data-boneid="${t.getAttribute('data-boneid')}"]`);
    return el && el.getAttribute('data-bonekey') === key;
  });
}
...
function getAllBoneLblsByKey(key) {
  return [...boneLabelLayer.querySelectorAll('text')].filter(t => {
    const id  = t.getAttribute('data-boneid');
    const ref = boneRegionLayer.querySelector(`[data-boneid="${id}"]`);
    return ref && ref.getAttribute('data-bonekey') === key;
  });
}
...
function showBoneInfo(key) {
  ...
  document.getElementById('infoContent').classList.remove('show');
  document.getElementById('subInfoBanner').classList.remove('visible');
  const bc = document.getElementById('boneInfoContent');
  bc.classList.remove('show');
  void bc.offsetWidth;
  bc.classList.add('show');
  // Populate
  document.getElementById('bRegion').textContent   = d.region;
  document.getElementById('bBoneType').textContent = d.boneType + ' BONE';
  document.getElementById('bName').textContent     = d.name;
  document.getElementById('bSci').textContent      = d.sci;
  document.getElementById('bDesc').textContent     = d.desc;
  document.getElementById('bFunc').textContent     = d.func;
  document.getElementById('bFact').textContent     = d.fact;
  // Joints chips
  document.getElementById('bJoints').innerHTML = (d.joints||[]).map(j =>
    `<span class="bone-joint-chip">${j}</span>`).join('');
  // Injuries
  const bInj = document.getElementById('bInjCards');
  if(bInj) {
    bInj.innerHTML = (d.injuries||[]).map(inj => `
      <div class="injury-card">
        <div class="injury-header">
          <span class="injury-name">${inj.name}</span>
          <span class="injury-sev ${inj.severity}">${inj.severity}</span>
  ...
  document.getElementById('infoPanel').scrollTop = 0;
  document.getElementById('stickyName').textContent    = d.name.toUpperCase();
  document.getElementById('stickySubName').textContent = d.boneType.toUpperCase() + ' BONE';
  document.getElementById('stickyHdr').classList.add('show');
}
...
document.addEventListener('click', function(e) {
  if (!e.target.closest('.layer-dd')) {
    document.querySelectorAll('.layer-dd.open').forEach(d => d.classList.remove('open'));
  }
});
...
function toggleLayer(layer) {
  if(layer === 'muscles') {
    ...
    if(!bonesVisible && selectedBoneKey) clearBoneSelection();
  }
}
...
(function _initMuscleSearch() {
  ...
  function _getResults(query) {
    ...
    if(!q) return SEARCH_INDEX.slice(0, 40);
...
(function _initAdvancedFilter() {
  ...
  function _totalActive() {
    return activeRegions.size + (activeFiber ? 1 : 0);
  }
  ...
  function _applyToSVG() {
    ...
    if(noRegionFilter && noFiberFilter) {
      // Remove all dimming
      document.querySelectorAll('.muscle-path.filter-dimmed')
        .forEach(el => el.classList.remove('filter-dimmed'));
      document.querySelectorAll('.muscle-label.filter-dimmed-lbl')
        .forEach(el => el.classList.remove('filter-dimmed-lbl'));
      return;
    }
    document.querySelectorAll('.muscle-path[data-key]').forEach(el => {
      const key  = el.getAttribute('data-key');
      const meta = META[key];
      if(!meta) return;
      let pass = true;
      if(!noRegionFilter) {
        pass = [...activeRegions].some(r => meta.regions.includes(r));
      }
    ...
    document.querySelectorAll('.muscle-label[data-key]').forEach(el => {
      const key  = el.getAttribute('data-key');
      const meta = META[key];
      if(!meta) { return; }
      let pass = true;
      if(!noRegionFilter) pass = [...activeRegions].some(r => meta.regions.includes(r));
  ...
  window._reapplyAdvancedFilter = _applyToSVG;
    ...
    setTimeout(applyHashSelection, 900);
  }
  ...
  function _hookOpenAnatomyView() {
    window.openAnatomyView = function(event) {
      ...
      const url = key
        ? `index.html#muscle=${encodeURIComponent(key)}`
        : 'index.html#anatomy';
  ...
  function _boot() {
    try {
      document.documentElement.dataset.anatomyLite =
        localStorage.getItem('grnd_anatomy_lite') === '1' ? '1' : '0';
    } catch(e) {}
    _injectHTML();
    _initViewer();

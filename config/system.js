...
(function(){
  ...
  /* Reveal text: leads with the punch (the flickering rank is fabricated),
     then circles back to confirm the one real number — the player's pts. */
  function _grndPopulateReveal(stats){
    var p1 = document.getElementById('ldb-reveal-p1');
    if (!p1) return;
    var fd = _grndStableFakeData();
    p1.innerHTML =
      '<span class="ldb-reveal-punch">Ranked <span id="ldb-reveal-rank" data-rank="' + fd.rank + '" data-total="' + fd.total + '">' +
      fd.rank.toLocaleString() + '</span>/' +
      '<span id="ldb-reveal-total">' + fd.total.toLocaleString() + '</span>. That number didn\'t exist ten seconds ago.</span> ' +
      'There was no "you" to rank until you tapped SEE RANK ANYWAY — the board assembled itself around your score right then, live, while you watched it happen. ' +
      'Your ' + stats.points.toLocaleString() + ' pts is the only real thing in that sentence. You earned every one of those.';
  }

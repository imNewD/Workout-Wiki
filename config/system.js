...
(function(){
  try {
    ...
    const validThemes = ['light','dark','pushup','pull','dip','squat','core','handstand','isometric','combo','warmup','stretch','sidequests','hof','pantheon','gym','cardio','beber'];
...
/* ═══════════════════════════════════════════════════════════════════════
   ②.6 UI THEME (liquid glass, etc.)
   Composable layer on top of color themes. Persisted separately.
   Storage key: grnd_ui_theme  Values: 'default' | 'glass'
   Public API: setUITheme(theme)
═══════════════════════════════════════════════════════════════════════ */
/* ┌─ @file: js/ui-theme.js ────────────────────────────────────────── */
function _updateUIThemeBtns() {
  ...
  var gb  = document.getElementById('uiGlassBtn');
  var desc = document.getElementById('ui-theme-desc');
  if (db)   db.classList.toggle('active',  cur === 'default' || !cur);
  if (gb)   gb.classList.toggle('active',  cur === 'glass');
  if (desc) desc.textContent = cur === 'glass' ? 'Liquid glass active' : 'Default layout active';
}
...
document.addEventListener('DOMContentLoaded', _updateUIThemeBtns);
/* └─ @end:  js/ui-theme.js ──────────────────────────────────────── */
    ...
    window.sendFeedback = function() {
      ...
      if (ta) {
        ...
        if (val === 'admin') {
          ...
          setLeaderboardAdminEnabled(true);
          if (window.showLeaderboardAdminButtons) window.showLeaderboardAdminButtons();
          ta.value = '';
...
(function () {
  ...
  /* ── Loader / Background stubs (image pickers — not lost) ─────── */
  function pickLoader() {}

...
(function () {
  ...
  const CLUSTERS = [
    ...
    /* Split from a single "PERSONALIZATION" cluster into two — the six UI
       skins and the four personal-content unlocks (loader/alarm/quote/
       backdrop) don't share a target (one reskins the whole app, the
       other lets you drop your own images/sounds/words into it), and
       crammed together they read as one big undifferentiated block.
       Keeping them physically apart (LAB/PERSONAL TOUCHES cascade down
       cols 2–3; UI STYLES sits over at cols 5–6) also means each keeps
       its own short hub-connector instead of one connector zigzagging
       across the whole map through all ten nodes. */
    {
      name: 'PERSONAL TOUCHES',
      color: '#d9b26b',
      nodes: [
        { key: 'loader', label: 'CUSTOM LOADER', icon: '🖼️', coord: [2, 3], kind: 'action',
        ...
        { key: 'audio', label: 'CUSTOM ALARM', icon: '🔊', coord: [3, 3], kind: 'action',
        ...
        { key: 'quote', label: 'CUSTOM QUOTE', icon: '💬', coord: [2, 4], kind: 'action',
        ...
        { key: 'background', label: 'CUSTOM BACKDROP', icon: '🏞️', coord: [3, 4], kind: 'action',
    ...
    },
    {
      ...
      color: '#c9a0dc',
      nodes: [
        /* Eight interface skins, each its own hex, gated by player level
           (10/25/50/75/100 — clay, spatial, brutal & arcade share the top
           tier, since there are 8 skins but only 5 requested tiers). Every
           skin already exists and works (setUITheme) — only the gate + map
        ...
        { key: 'spatial', label: 'SPATIAL', icon: '🌌', coord: [6, 5], kind: 'action',
          ...
          progress: () => _capProgress(Math.min((typeof getPlayerLevelSummary === 'function' ? getPlayerLevelSummary().level : 0), 100), 100) },
        { key: 'brutal', label: 'NEUBRUTALISM', icon: '🧱', coord: [5, 6], kind: 'action',
          desc: 'Raw blocks, thick ink, hard shadows. Reach Player Level 100.',
          unlocked: () => window.isUIStyleUnlocked('brutal'),
          btnLabel: () => (document.documentElement.dataset.uiTheme === 'brutal' ? 'ACTIVE' : 'ACTIVATE'),
          activate: () => window.setUITheme?.('brutal'),
          hasValue: () => document.documentElement.dataset.uiTheme === 'brutal',
          progress: () => _capProgress(Math.min((typeof getPlayerLevelSummary === 'function' ? getPlayerLevelSummary().level : 0), 100), 100) },
        { key: 'arcade', label: 'ARCADE', icon: '👾', coord: [6, 6], kind: 'action',
          desc: '8-bit cabinet — pixel frames & scanlines. Reach Player Level 100.',
          unlocked: () => window.isUIStyleUnlocked('arcade'),
          btnLabel: () => (document.documentElement.dataset.uiTheme === 'arcade' ? 'ACTIVE' : 'ACTIVATE'),
          activate: () => window.setUITheme?.('arcade'),
          hasValue: () => document.documentElement.dataset.uiTheme === 'arcade',
          progress: () => _capProgress(Math.min((typeof getPlayerLevelSummary === 'function' ? getPlayerLevelSummary().level : 0), 100), 100) },
      ],
    },
    {
      ...
      color: '#9ad1d4',
      nodes: [
        /* Unlock-celebration style pickers. Each hex is one animation
           style; its info panel shows 3 independent toggles — BASIC,
           HOF, PANTHEON — rather than a single ACTIVATE button (see the
           kind:'style' branch in openAchvInfo below). Toggling a
           category ON here makes this style play for that category's
           exercise unlocks, and turns the category off on whichever
           other style hex previously had it (only one style can be
           active per category at a time). PANTHEON is hidden until
           Pantheon is unlocked. Always available — a free preference,
           not an achievement — so unlocked() is always true. */
        { key: 'ub_glassbreak', label: 'GLASS BREAK', icon: '🔷', coord: [7, 3], kind: 'style', styleId: 'glass-break',
          desc: 'Classic shard shatter. The default for every category.',
          unlocked: () => true, progress: () => null },
        { key: 'ub_slash', label: 'WORLD CUTTING SLASH', icon: '🗡️', coord: [8, 3], kind: 'style', styleId: 'slash',
          desc: 'The card is cut apart by a flurry of blade strikes.',
          unlocked: () => true, progress: () => null },
        { key: 'ub_landed', label: 'LANDED', icon: '☄️', coord: [7, 4], kind: 'style', styleId: 'landed',
          desc: 'The card drops in like a meteor and buries itself on impact.',
          unlocked: () => true, progress: () => null },
        { key: 'ub_melt', label: 'SUMMER MELT', icon: '💧', coord: [8, 4], kind: 'style', styleId: 'melt',
          desc: 'The card itself turns to liquid before reforming.',
          unlocked: () => true, progress: () => null },
        { key: 'ub_unbroken', label: 'UNBROKEN', icon: '📦', coord: [7, 5], kind: 'style', styleId: 'unbroken',
          desc: 'The card is crushed down, then pops open with a loud snap.',
          unlocked: () => true, progress: () => null },
      ],
    },
    {
      name: 'PROGRESSION VIEWS',
      color: '#6ab7e0',
      nodes: [
        { key: 'treeview_tree', label: 'TREE', icon: '🌳', coord: [9, 3], kind: 'action',
          desc: 'The classic progression tree — tiers, family lanes, side-quest branches.',
          unlocked: () => true,
          btnLabel: () => ((window._treeViewStyleCurrent?.() || 'tree') === 'tree' ? 'ACTIVE' : 'ACTIVATE'),
          activate: () => window.setTreeViewStyle?.('tree'),
          hasValue: () => (window._treeViewStyleCurrent?.() || 'tree') === 'tree',
          progress: () => null },
      ],
    },
  ...
  const HEX_SIZE = 42;                              // center-to-corner radius — actual button size, unchanged
  const HEX_W = Math.sqrt(3) * HEX_SIZE;             // ~72.7 — button width
  const HEX_H = HEX_SIZE * 2;                        // 84    — button height
  const ROW_SPACING = HEX_SIZE * 1.5;                // 63
  const PAD = HEX_W * 1.3;                           // breathing room (legendary hexes render larger than PAD*1)
  /* GRID_GAP spaces out hex CENTERS beyond perfect tessellation (which is
     what HEX_W/ROW_SPACING alone produce — adjacent hexes touch edge to
     edge with zero gap). The hexes themselves stay the same size; only
     the distance between them grows, so the map reads as a spaced-out
     skill web instead of a packed honeycomb. */
  const GRID_GAP = 1.28;
  const STEP_W   = HEX_W * GRID_GAP;
  const STEP_ROW = ROW_SPACING * GRID_GAP;

  function _hexToPixel([col, row]) {
    const x = col * STEP_W + (Math.abs(row % 2) === 1 ? STEP_W / 2 : 0);
    const y = row * STEP_ROW;
    return { x, y };
  }
  ...
  /* ── Info panel ──────────────────────────────────────────────────── */
  var _openStyleNode = null; // the currently-open kind:'style' node, so _toggleAchvStyleCat knows which style it's toggling
  ...
  window.openAchvInfo = function (key) {
    ...
    const styleToggles = document.getElementById('achv-info-style-toggles');

    if (isStyle) {
      _openStyleNode = node;
      btn.style.display = 'none';
      resetBtn.style.display = 'none';
      if (styleToggles) { styleToggles.style.display = 'flex'; _syncAchvStyleToggles(); }
    } else {
      _openStyleNode = null;
      if (styleToggles) styleToggles.style.display = 'none';
      if (unlocked) {
        btn.style.display = '';
        if (isAction) {
  ...
  /* Syncs the 3 BASIC/HOF/PANTHEON toggle buttons for the currently-open
     style hex against window.getUnlockAnimForCategory (assets/unlock-animations.js).
     PANTHEON stays hidden until Pantheon itself is unlocked. */
  function _syncAchvStyleToggles() {
    if (!_openStyleNode || typeof window.getUnlockAnimForCategory !== 'function') return;
    const styleId = _openStyleNode.styleId;
    const pantheonUnlocked = typeof window.isPantheonModeUnlocked === 'function' && window.isPantheonModeUnlocked();
    ['basic', 'hof', 'pantheon'].forEach(function (cat) {
      const btn = document.getElementById('achv-info-style-toggle-' + cat) ||
        document.querySelector('#achv-info-style-toggles [data-cat="' + cat + '"]');
      if (!btn) return;
      if (cat === 'pantheon' && !pantheonUnlocked) { btn.style.display = 'none'; return; }
      btn.style.display = '';
      const on = window.getUnlockAnimForCategory(cat) === styleId;
      btn.textContent = cat.toUpperCase() + (on ? ' ✓' : '');
      btn.classList.toggle('menu-action-btn-primary', on);
    });
  }

  window._toggleAchvStyleCat = function (cat) {
    if (!_openStyleNode || typeof window.setUnlockAnimForCategory !== 'function') return;
    const styleId = _openStyleNode.styleId;
    const isOn = window.getUnlockAnimForCategory(cat) === styleId;
    // Toggling off falls back to the default (Glass Break) for that category;
    // toggling on assigns this style, replacing whatever was active before.
    window.setUnlockAnimForCategory(cat, isOn ? 'glass-break' : styleId);
    _syncAchvStyleToggles();
  };
